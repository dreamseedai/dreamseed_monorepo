...
"""
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert mathematician and LaTeX specialist. Convert MathML to MathLive LaTeX format with high accuracy.",
                    },
    ...
    def process_content_batch(
        self, content_list: List[str], batch_size: int = 10
    ) -> List[Dict]:
        ...
        for i in range(0, len(content_list), batch_size):
            ...
            print(
                f"Processing batch {i//batch_size + 1}/{(len(content_list) + batch_size - 1)//batch_size}"
            )
            ...
            for content in batch:
                ...
                for mathml, position in mathml_expressions:
                    ...
                    if conversion_result.converted_latex:
                        if (
                            conversion_result.conversion_notes
                            and "display" in conversion_result.conversion_notes.lower()
                        ):
                            latex_wrapper = f"\\[{conversion_result.converted_latex}\\]"
                        else:
                            latex_wrapper = f"\\({conversion_result.converted_latex}\\)"

                        content_result["converted_content"] = content_result[
                            "converted_content"
                        ].replace(mathml, latex_wrapper)
