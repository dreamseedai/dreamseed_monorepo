// apps/teacher_front/src/app/teacher/class/page.tsx
"use client";
...
export default function TeacherClassPage() {
  ...
  const [data, setData] = useState<TeacherClassListResponse | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null);
      try {
        const res = await fetchTeacherClassList(subject, className || undefined, windowDays);
        if (!cancelled) setData(res);
      } catch (err) {
        if (!cancelled) {
          const message = err instanceof Error ? err.message : "학급 리스트를 불러오는 중 오류가 발생했습니다.";
          setError(message);
        }
      } finally {
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-600">
            담당 학급 학생들의 IRT/CAT 능력치와 학습 현황을 확인하고 리포트 코멘트를 작성하세요.
          </p>
        </div>
      ...
      {loading && (
        <p className="text-sm text-gray-500">학급 데이터를 불러오는 중...</p>
      )}
        ...
        <div>
          <div className="mb-3 rounded-lg border border-blue-200 bg-blue-50 p-3">
            <div className="flex items-center gap-2 text-sm">
              ...
              {data.className && (
                <span className="text-blue-700">| {data.className}</span>
              )}
              <span className="text-blue-600">| {data.subject}</span>
            </div>
          </div>
