...
if (!is.null(data$bayesian_posterior) && length(data$bayesian_posterior) > 0) {
  bp <- data$bayesian_posterior
  ...
  if (!is.na(P)) {
    ...
    cat("Bayesian posterior P value not available.\n")
  }
} else {
  cat("No Bayesian posterior data available for this week.\n")
}
```
