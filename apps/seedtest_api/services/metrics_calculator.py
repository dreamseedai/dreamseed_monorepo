...
class MetricsCalculator:
    ...
    def calculate_improvement_index(
        ...
        # 향상지수 계산
        improvement_index = (
            theta_improvement * exposure_factor * confidence_penalty_factor
        )

        return max(0.0, improvement_index)

    def calculate_efficiency_index(
        self, user_id: str, topic_id: Optional[str] = None, weeks: int = 2
    ) -> float:
        """시간효율 E_t 계산
    ...
    def calculate_recovery_index(
        ...
        weak_concepts = self._identify_weak_concepts(
            user_id, topic_id, weeks, weakness_threshold
        )
        ...
        for concept in weak_concepts:
            ...
            successful_transitions += sum(
                1
                for prev_correct, curr_correct in transitions
    ...
    def calculate_engagement_index(
        self, user_id: str, weeks: int = 2, weights: Optional[Dict[str, float]] = None
    ) -> float:
        """참여도 A_t 계산
        ...
        # 학습간격 (일관성 점수)
        learning_interval = self._calculate_learning_interval_consistency(
            user_id, weeks
        )
    ...
    def calculate_dropout_risk(
        self, user_id: str, days_threshold: int = 14, baseline_risk: float = 0.1
    ) -> float:
        """이탈위험 S(t) 계산
    ...
    def _get_theta_changes(
        self, user_id: str, topic_id: Optional[str], weeks: int
    ) -> List[float]:
        """θ 변화량 추적 (주간 평균).
        ...
        rows = (
            self.db.execute(sql, {"uid": user_id, "start": start_dt}).mappings().all()
        )
        return [float(r["avg_theta"] or 0.0) for r in rows]

    def _get_exposure_count(
        self, user_id: str, topic_id: Optional[str], weeks: int
    ) -> int:
        ...
        row = (
            self.db.execute(sql, {"uid": user_id, "start": start_dt}).mappings().first()
        )
    ...
    def _identify_weak_concepts(
        self, user_id: str, topic_id: Optional[str], weeks: int, threshold: float
    ) -> List[str]:
            ...
            try:
                ...
                if isinstance(topics, list) and topics:
                    weak = [
                        str(t.get("topic"))
                        for t in topics
                        if float(t.get("accuracy") or 0.0) <= float(threshold)
                    ]
                    return weak[:10]
            except Exception:
                continue
    ...
    def _get_concept_transitions(
        self, user_id: str, concept: str, weeks: int
    ...
    def _calculate_learning_interval_consistency(
        self, user_id: str, weeks: int
    ) -> float:
    ...
    def _get_risk_covariates(self, user_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "session_consistency": self._calculate_learning_interval_consistency(
                user_id, weeks=4
            ),
        }
    ...
    def upsert_weekly_kpi(
        self, user_id: str, week_start: date, kpis: Dict[str, Any]
    ) -> None:
    ...
    def _adjust_risk_with_covariates(
        self, base_risk: float, covariates: Dict[str, Any]
    ) -> float:
        """공변량 기반 위험도 조정"""
