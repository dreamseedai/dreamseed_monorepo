...
policy_deny_total = Counter(
    "policy_deny_total", "Total policy denials", ["action", "role", "phase", "reason"]
)

policy_allow_total = Counter(
    "policy_allow_total", "Total policy allows", ["action", "role", "phase"]
)
...
feature_flag_checks_total = Counter(
    "feature_flag_checks_total", "Total feature flag checks", ["flag_name", "result"]
)

feature_flag_enabled = Gauge(
    "feature_flag_enabled", "Feature flag enabled status", ["flag_name", "phase"]
)

# IRT 드리프트 메트릭
irt_drift_detections_total = Counter(
    "irt_drift_detections_total", "Total IRT drift detections", ["status"]
)

irt_drift_flagged_items = Gauge(
    "irt_drift_flagged_items", "Number of flagged items in IRT drift detection"
)

irt_drift_detection_duration_seconds = Histogram(
    "irt_drift_detection_duration_seconds", "IRT drift detection duration"
)
...
def record_policy_evaluation(
    action: str, role: str, phase: str, result: str, duration: float
):
    """정책 평가 메트릭 기록"""
    policy_evaluations_total.labels(
        action=action, role=role, phase=phase, result=result
    ).inc()
    policy_evaluation_duration_seconds.labels(action=action, phase=phase).observe(
        duration
    )
    ...
    if result == "allow":
        ...
        policy_deny_total.labels(
            action=action, role=role, phase=phase, reason="policy_violation"
        ).inc()


def record_bundle_reload(bundle_id: str, phase: str, status: str, duration: float):
    """번들 리로드 메트릭 기록"""
    governance_bundle_reload_total.labels(
        bundle_id=bundle_id, phase=phase, status=status
    ).inc()
    governance_bundle_reload_duration_seconds.labels(
        bundle_id=bundle_id, phase=phase
    ).observe(duration)
...
def record_feature_flag_check(flag_name: str, result: bool, phase: str):
    ...
    feature_flag_enabled.labels(flag_name=flag_name, phase=phase).set(
        1 if result else 0
    )
