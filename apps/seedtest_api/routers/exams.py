...
async def next_question(
    exam_id: str, payload: NextStepRequest, user=Depends(require_scopes("exam:write"))
) -> NextQuestionResponse:
    ...
    if payload.last_question_id == "q_final":
        return NextQuestionResponse(
            ...
            question=None,
            result={"score": 10, "correct": 8, "incorrect": 2, "duration_sec": 500},
        )
...
async def create_session(
    exam_id: str, req: CreateExamRequest, user=Depends(require_scopes("exam:write"))
) -> CreateExamResponse:
...
async def submit_answer_route(
    ...
    res = submit_answer(
        session_id, payload.question_id, payload.answer or "", payload.elapsed_time
    )
    if res.get("finished"):
...
@router.get(
    "/exams/{exam_id}/sessions/{session_id}/next", response_model=NextQuestionResponse
)
async def get_next_question(
    exam_id: str, session_id: str, user=Depends(require_scopes("exam:write"))
) -> NextQuestionResponse:
    q = select_next(session_id)
    if not q:
        return NextQuestionResponse(
            done=True, next_difficulty=None, question=None, result=None
        )
    return NextQuestionResponse(
        done=False, next_difficulty=None, question=QuestionOut(**q), result=None
    )
...
async def create_exam(
    body: CreateExamRequest, payload=Depends(require_scopes("exam:write"))
) -> CreateExamResponse:
...
@router.get(
    "/exams/{session_id}/next",
    response_model=NextQuestionResponse,
    summary="Get next question",
)
async def next_question_by_session(
    session_id: str, payload=Depends(require_scopes("exam:read"))
) -> dict[str, Any]:
    q = select_next(session_id)
    if q is None:
...
@router.post(
    "/exams/{session_id}/response",
    response_model=NextQuestionResponse,
    summary="Submit answer and get next",
)
async def submit_by_session(
    ...
    res = submit_answer(
        session_id, body.question_id, body.answer or "", body.elapsed_time
    )
    if "error" in res:
