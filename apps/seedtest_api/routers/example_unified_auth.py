...
async def create_class(name: str, user: UserContext = Depends(get_current_user)):
    """Create a new class.
...
async def get_exam_session(
    ...
    verify_session_access(
        session_user_id=session["user_id"], session_org_id=session["org_id"], user=user
    )
...
async def get_dashboard(user: UserContext = Depends(get_current_user)):
    ...
    if user.is_admin():
        ...
        return {"type": "viewer_dashboard", "message": "Read-only access"}
