...
async def calculate_efficiency_index(
    ...
    try:
        ...
        efficiency_index = calculator.calculate_efficiency_index(
            user_id=user_id, topic_id=request.topic_id, weeks=request.weeks
        )
...
async def calculate_engagement_index(
    ...
    try:
        ...
        engagement_index = calculator.calculate_engagement_index(
            user_id=user_id, weeks=request.weeks, weights=request.weights
        )
...
async def calculate_topic_ability(
    ...
    try:
        ...
        return TopicAbilityResponse(
            topic_id=request.topic_id, theta=theta, standard_error=se, method=method
        )

    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"토픽별 숙련도 계산 실패: {str(e)}"
        )
...
async def get_topic_abilities(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"토픽별 숙련도 조회 실패: {str(e)}"
        )
...
async def get_weak_topics(
    ...
    try:
        ...
        weak_topics = service.identify_weak_topics(
            user_id=user_id, threshold=threshold, min_responses=min_responses
        )
