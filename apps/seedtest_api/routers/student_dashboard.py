...
class DashboardOut(BaseModel):
    ...
    week_growth: float = Field(
        ..., description="7-day average theta delta (IRT ability change)"
    )
    today_mood: Optional[str] = Field(
        None, description="Today's mood: 'happy' | 'neutral' | 'sad'"
    )
    streak_days: int = Field(..., description="Consecutive days with learning activity")
    goals: List[GoalOut] = Field(
        ..., description="Active (not done) goals, max 5 recent"
    )
    ai_message: Optional[str] = Field(
        None, description="AI-generated encouragement message"
    )
    ai_tone: Optional[str] = Field(
        None, description="Message tone: 'warm' | 'gentle' | 'energetic'"
    )
...
def get_dashboard(
    db: Session = Depends(get_db), user: UserContext = Depends(require_role("student"))
):
    ...
    logs_7d = db.execute(
        select(func.avg(StudentDailyLog.theta_delta)).where(
            and_(
...
class MoodIn(BaseModel):
    ...
    mood: str = Field(
        ...,
        pattern="^(happy|neutral|sad)$",
        description="Mood: 'happy' | 'neutral' | 'sad'",
    )
    note: Optional[str] = Field(
        None, max_length=512, description="Optional note about mood"
    )
