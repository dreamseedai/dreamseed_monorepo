...
def update_question_meta_batch(batch_size: int = 100, dry_run: bool = False) -> dict:
    ...
    with get_session() as session:
        ...
        print(f"[INFO] Found {len(rows)} recent item params; processing in batches of {batch_size}")
        ...
        for idx, row in enumerate(rows, 1):
            ...
            if not question_exists:
                stats["skipped"] += 1
                if idx % 100 == 0:
                    print(f"[PROGRESS] {idx}/{len(rows)} processed; updated={stats['updated']}, skipped={stats['skipped']}")
                continue
            
            try:
                # Extract IRT parameters
                params = row["params"] if isinstance(row["params"], dict) else json.loads(str(row["params"]))
...
def main() -> int:
    ...
    print(f"[INFO] Starting question.meta.irt update; batch_size={batch_size}, dry_run={dry_run}")
