...
def update_weekly_kpi_with_p_s(session, user_id: str, week_start: date, P: Optional[float], S: Optional[float]) -> None:
...
def main(threshold_days: Optional[int] = None, dry_run: bool = False) -> int:
    ...
    threshold = threshold_days if threshold_days is not None else INACTIVITY_THRESHOLD_DAYS
    today = date.today()
    ...
    try:
        with get_session() as session:
            ...
            print(f"[INFO] Found {len(inactive_users)} inactive users (threshold={threshold} days); dry_run={dry_run}")
                ...
                try:
                    ...
                    if ps_values:
                        ...
                        if os.getenv("DEBUG", "").lower() == "true":
                            print(f"[DEBUG] OK user={user_id} P={ps_values.get('P')} S={ps_values.get('S')}")
                    else:
                        failed += 1
        ...
        print(f"[INFO] Summary: processed={processed}, failed={failed}, threshold={threshold} days, duration_ms={duration_ms}")
...
def cli() -> None:
    ...
    parser = argparse.ArgumentParser(description="Detect inactive users and trigger P/S recalculation")
    parser.add_argument(
        "--threshold",
