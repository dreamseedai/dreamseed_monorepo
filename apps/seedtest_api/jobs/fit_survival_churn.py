...
async def fit_survival_model(
    ...
    print(
        f"[INFO] Fitting survival model (lookback={lookback_days} days, event_threshold={event_threshold_days} days)"
    )
    ...
    try:
        ...
        if result.get("status") == "error" or result.get("error"):
            print(
                f"[WARN] Survival fit returned error: {result.get('error') or result.get('message')}"
            )
            return
    ...
    run_id = (
        result.get("run_id")
        or f"survival-{datetime.utcnow().strftime('%Y%m%d-%H%M%S')}"
    )
    ...
    print(
        f"[INFO] Survival model fitted: n={model_meta.get('n')}, family={model_meta.get('family')}, concordance={model_meta.get('concordance')}"
    )

    # Build risk score map
    risk_scores = {
        str(p.get("user_id")): float(p.get("risk_score", 0.0))
        for p in preds
    ...
    with get_session() as s:
        ...
        s.execute(
            stmt_meta,
            {
                ...
                "event_threshold_days": int(
                    model_meta.get("event_threshold_days", event_threshold_days)
                ),
                ...
                try:
                    ...
                    s.execute(
                        stmt_risk,
                        {
                            ...
                            "hazard_ratio": (
                                float(p.get("hazard_ratio", 1.0))
                                if p.get("hazard_ratio") is not None
                                else None
                            ),
                            "rank_percentile": (
                                float(p.get("rank_percentile", 0.0))
                                if p.get("rank_percentile") is not None
                                else None
                            ),
                        },
                    )
                    ...
                    print(
                        f"[WARN] Failed to upsert survival risk for {p.get('user_id')}: {e}"
                    )
