...
def test_compute_improvement_index_no_data(mock_session):
    ...
    result = compute_improvement_index(mock_session, "user1", date(2025, 1, 20), window_days=14)
...
def test_compute_improvement_index_positive_delta(mock_session):
    ...
    def mock_load_attempts(session, user_id, start, end):
        ...
        if end > datetime(2025, 1, 15, tzinfo=timezone.utc):
            ...
            return [Attempt(None, None, True if i < 10 else False, None, None, None, None, None) for i in range(20)]
        else:
            # Previous period: lower accuracy (5/20 = 0.25)
            return [Attempt(None, None, True if i < 5 else False, None, None, None, None, None) for i in range(20)]
...
def test_compute_time_efficiency_no_data(mock_session):
    ...
    result = compute_time_efficiency(mock_session, "user1", date(2025, 1, 20), window_days=28)
    assert result is None
...
def test_compute_time_efficiency_improvement(mock_session):
    ...
    def mock_load_attempts(session, user_id, start, end):
        if end > datetime(2025, 1, 1, tzinfo=timezone.utc):
            ...
            return [
                Attempt(None, None, True, datetime.now(tz=timezone.utc), 1000, None, None, None)
                for _ in range(5)
            ]
        else:
            # Previous: slower (2000ms)
            return [
                Attempt(None, None, True, datetime.now(tz=timezone.utc), 2000, None, None, None)
                for _ in range(5)
            ]
...
def test_compute_time_efficiency_zero_median(mock_session):
    ...
    def mock_load_attempts(session, user_id, start, end):
        return [
            Attempt(None, None, True, datetime.now(tz=timezone.utc), 0, None, None, None)
            for _ in range(5)
        ]
...
def test_compute_recovery_rate_no_prev_incorrect(mock_session):
    ...
    def mock_load_attempts(session, user_id, start, end):
        ...
        return [
            Attempt("q1", "topic1", True, datetime.now(tz=timezone.utc), None, None, None, None)
            for _ in range(5)
        ]

    metrics_svc.load_attempts = mock_load_attempts
    result = compute_recovery_rate(mock_session, "user1", date(2025, 1, 20), window_days=28)
    assert result is None
...
def test_compute_recovery_rate_successful_recovery(mock_session):
    ...
    def mock_load_attempts(session, user_id, start, end):
        if end > datetime(2025, 1, 1, tzinfo=timezone.utc):
            ...
            return [
                Attempt("q1", "topic1", True, datetime.now(tz=timezone.utc), None, None, None, None),
                Attempt("q2", "topic2", True, datetime.now(tz=timezone.utc), None, None, None, None),
            ]
        else:
            # Previous: incorrect
            return [
                Attempt("q1", "topic1", False, datetime.now(tz=timezone.utc), None, None, None, None),
                Attempt("q2", "topic2", False, datetime.now(tz=timezone.utc), None, None, None, None),
            ]
...
def test_compute_engagement_no_data(mock_session):
    ...
    metrics_svc.load_attempts = mock_load_attempts
    result = compute_engagement(mock_session, "user1", date(2025, 1, 20), window_days=28)
    assert result is None
...
def test_compute_engagement_with_missing_components(mock_session):
    ...
    def mock_load_attempts(session, user_id, start, end):
        ...
        return [
            Attempt(
                "q1", "topic1", True, datetime.now(tz=timezone.utc), None, None, "sess1", None
            ),
            Attempt(
                "q2", "topic2", False, datetime.now(tz=timezone.utc), None, None, "sess1", None
            ),
        ]
...
def test_compute_goal_attainment_probability_none_when_no_ability(mock_session, monkeypatch):
    monkeypatch.setattr(metrics_svc, "load_user_ability_summary", lambda session, uid: None)
    p = metrics_svc.compute_goal_attainment_probability(mock_session, "user1", target=0.0)
    assert p is None


def test_compute_goal_attainment_probability_sd_le_zero(mock_session, monkeypatch):
    monkeypatch.setattr(metrics_svc, "load_user_ability_summary", lambda session, uid: (0.0, 0.0))
    p = metrics_svc.compute_goal_attainment_probability(mock_session, "user1", target=0.0)
    assert p is None


def test_compute_goal_attainment_probability_normal_valid(mock_session, monkeypatch):
    monkeypatch.setattr(metrics_svc, "load_user_ability_summary", lambda session, uid: (0.0, 1.0))
    p = metrics_svc.compute_goal_attainment_probability(mock_session, "user1", target=0.0)
    assert p is not None and 0.49 <= p <= 0.51


def test_compute_goal_attainment_probability_bayesian_flag_fallback(mock_session, monkeypatch):
...
def test_calculate_and_store_weekly_kpi_sets_P_when_available(mock_session, monkeypatch):
    ...
    monkeypatch.setattr(metrics_svc, "compute_goal_attainment_probability", lambda session, uid, target=None: 0.75)
...
def test_compute_churn_risk_near_zero_when_active(mock_session, monkeypatch):
    ...
    def fake_stats(session, user_id, lookback_days=56):
        return {"last_seen": date(2025, 1, 6), "sessions": 12, "mean_gap_days": 0.0, "first_seen": date(2024, 12, 1)}
...
def test_compute_churn_risk_near_one_when_stale(mock_session, monkeypatch):
    ...
    def fake_stats(session, user_id, lookback_days=56):
        return {"last_seen": date(2025, 1, 6), "sessions": 0, "mean_gap_days": 7.0, "first_seen": date(2024, 10, 1)}
    monkeypatch.setattr(m, "load_user_session_stats", fake_stats)
    r = m.compute_churn_risk(mock_session, "U1", as_of=date(2025, 1, 20))  # 14 days later
...
def test_compute_churn_risk_sessions_reduce_risk(mock_session, monkeypatch):
    ...
    def stats_low(session, user_id, lookback_days=56):
        return {"last_seen": date(2025, 1, 6), "sessions": 1, "mean_gap_days": 3.0, "first_seen": date(2024, 12, 1)}
    def stats_high(session, user_id, lookback_days=56):
        return {"last_seen": date(2025, 1, 6), "sessions": 12, "mean_gap_days": 3.0, "first_seen": date(2024, 12, 1)}
...
def test_compute_churn_risk_gap_increases_risk(mock_session, monkeypatch):
    ...
    def stats_small(session, user_id, lookback_days=56):
        return {"last_seen": date(2025, 1, 6), "sessions": 5, "mean_gap_days": 1.0, "first_seen": date(2024, 12, 1)}
    def stats_large(session, user_id, lookback_days=56):
        return {"last_seen": date(2025, 1, 6), "sessions": 5, "mean_gap_days": 6.0, "first_seen": date(2024, 12, 1)}
...
def test_compute_churn_risk_horizon_flag_effect(mock_session, monkeypatch):
    ...
    def fake_stats(session, user_id, lookback_days=56):
        return {"last_seen": date(2025, 1, 6), "sessions": 2, "mean_gap_days": 2.0, "first_seen": date(2024, 12, 1)}
    monkeypatch.setattr(m, "load_user_session_stats", fake_stats)
...
def test_calculate_and_store_weekly_kpi_sets_S_when_available(mock_session, monkeypatch):
    from seedtest_api.services import metrics as m
