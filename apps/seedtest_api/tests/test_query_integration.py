...
pytestmark = pytest.mark.skipif(SKIP_INTEGRATION, reason="Integration tests disabled (set RUN_INTEGRATION_TESTS=1 to enable)")
...
def _compose_up():
  subprocess.run(["docker", "compose", "-f", str(ROOT / "docker-compose.db.yml"), "up", "-d"], check=True)
  _wait_port("127.0.0.1", 5432, 60)


def _compose_down():
  subprocess.run(["docker", "compose", "-f", str(ROOT / "docker-compose.db.yml"), "down", "-v"], check=False)


def _alembic_upgrade(tags_kind: str):
  os.environ["DATABASE_URL"] = "postgresql+psycopg2://user:pass@127.0.0.1:5432/dreamseed_db"
