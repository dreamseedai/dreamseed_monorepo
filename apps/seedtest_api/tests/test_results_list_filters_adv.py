...
if not os.getenv("DATABASE_URL"):
    pytest.skip(
        "DATABASE_URL not set; skipping advanced filters tests", allow_module_level=True
    )
...
def test_status_csv_and_created_updated_ranges(monkeypatch):
    ...
    with get_session() as s:
        s.execute(
            text(
                "UPDATE exam_results SET created_at=:t, updated_at=:t, status='pending' WHERE session_id='asess1'"
            ),
            {"t": t0},
        )
        s.execute(
            text(
                "UPDATE exam_results SET created_at=:t, updated_at=:t, status='failed'  WHERE session_id='asess2'"
            ),
            {"t": t1},
        )
        s.execute(
            text(
                "UPDATE exam_results SET created_at=:t, updated_at=:t, status='ready'   WHERE session_id='asess3'"
            ),
            {"t": t2},
        )
    ...
    r2 = client.get(
        ...
        params={"created_from": (t1 - timedelta(hours=1)).isoformat()},
    )
    ...
    r3 = client.get(
        "/api/seedtest/results",
        params={"updated_to": (t1 + timedelta(hours=1)).isoformat()},
    )
