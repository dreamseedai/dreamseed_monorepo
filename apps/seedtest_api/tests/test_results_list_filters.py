...
if not os.getenv("DATABASE_URL"):
    pytest.skip(
        "DATABASE_URL not set; skipping DB-backed listing filters tests",
...
def test_results_list_filters_and_sort(monkeypatch):
    ...
    with get_session() as s:
        s.execute(
            text("UPDATE exam_results SET status='failed' WHERE session_id=:sid"),
            {"sid": "fsess2"},
        )
    ...
    r2 = client.get(
        ...
        params={"sort_by": "created_at", "order": "asc", "limit": 50},
    )
    ...
    r3 = client.get(
        "/api/seedtest/results", params={"date_from": "2000-01-01T00:00:00Z"}
    )
