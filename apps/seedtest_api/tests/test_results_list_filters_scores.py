...
if not os.getenv("DATABASE_URL"):
    pytest.skip(
        "DATABASE_URL not set; skipping score/last_n_days filters tests",
...
def test_min_max_scaled_filters(monkeypatch):
    ...
    # both min and max
    r3 = client.get(
        "/api/seedtest/results", params={"min_score_scaled": 15, "max_score_scaled": 30}
    )
...
def test_last_n_days_convenience(monkeypatch):
    ...
    with get_session() as s:
        s.execute(
            text("UPDATE exam_results SET updated_at=:t WHERE session_id='dsess1'"),
            {"t": t_old},
        )
        s.execute(
            text("UPDATE exam_results SET updated_at=:t WHERE session_id='dsess2'"),
            {"t": t_mid},
        )
        s.execute(
            text("UPDATE exam_results SET updated_at=:t WHERE session_id='dsess3'"),
            {"t": t_new},
        )
...
def test_min_max_raw_filters(monkeypatch):
    ...
    # both min and max
    r3 = client.get(
        "/api/seedtest/results", params={"min_score_raw": 10, "max_score_raw": 20}
    )
    assert r3.status_code == 200
...
def test_last_n_hours_and_weeks(monkeypatch):
    ...
    with get_session() as s:
        s.execute(
            text("UPDATE exam_results SET updated_at=:t WHERE session_id='hsess1'"),
            {"t": h_old},
        )
        s.execute(
            text("UPDATE exam_results SET updated_at=:t WHERE session_id='hsess2'"),
            {"t": h_mid},
        )
        s.execute(
            text("UPDATE exam_results SET updated_at=:t WHERE session_id='hsess3'"),
            {"t": h_new},
        )
    ...
    r1 = client.get(
        "/api/seedtest/results",
        params={"last_n_hours": 2, "sort_by": "updated_at", "order": "asc"},
    )
    assert r1.status_code == 200
    ...
    r2 = client.get(
        "/api/seedtest/results",
        params={"last_n_weeks": 2, "sort_by": "updated_at", "order": "asc"},
    )
    assert r2.status_code == 200
    ids2 = [it["session_id"] for it in r2.json().get("items", [])]
...
def test_score_equality_and_created_last_n(monkeypatch):
    ...
    with get_session() as s:
        s.execute(
            text("UPDATE exam_results SET created_at=:t WHERE session_id='esess1'"),
            {"t": c_old},
        )
        s.execute(
            text("UPDATE exam_results SET created_at=:t WHERE session_id='esess2'"),
            {"t": c_mid},
        )
        s.execute(
            text("UPDATE exam_results SET created_at=:t WHERE session_id='esess3'"),
            {"t": c_new},
        )

    r3 = client.get(
        "/api/seedtest/results",
        params={"created_last_n_days": 2, "sort_by": "created_at", "order": "asc"},
    )
    assert r3.status_code == 200
