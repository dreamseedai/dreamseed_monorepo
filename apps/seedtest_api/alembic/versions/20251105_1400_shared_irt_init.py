...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            unique=True,
            nullable=False,
            comment="Human-readable item ID",
        ),
        ...
        sa.Column(
            ...
            sa.Text,
            nullable=False,
            comment="Language: en, ko, zh-Hans, zh-Hant",
        ),
        sa.CheckConstraint(
            "lang IN ('en','ko','zh-Hans','zh-Hant')", name="items_lang_chk"
        ),
        sa.Column("stem_rich", postgresql.JSONB, comment="Item stem with HTML/LaTeX"),
        sa.Column(
            "options_rich", postgresql.JSONB, comment="Answer choices with rich content"
        ),
        ...
        sa.Column(
            ...
            comment="Topic classifications",
        ),
        sa.Column(
            "subtopic_tags",
            postgresql.ARRAY(sa.Text),
            server_default=sa.text("'{}'"),
            comment="Subtopic classifications",
        ),
        sa.Column(
            "curriculum_tags",
            postgresql.ARRAY(sa.Text),
            server_default=sa.text("'{}'"),
            comment="Curriculum alignments",
        ),
        sa.Column(
            "metadata",
            postgresql.JSONB,
        ...
        sa.Column(
            ...
            comment="Total exposure count for CAT",
        ),
        sa.Column(
            "created_at",
    ...
    op.create_index(
        "idx_items_id_str", "items", ["id_str"], unique=True, schema="shared_irt"
    )
    op.create_index(
        "idx_items_anchor",
        "items",
    ...
    op.create_table(
        ...
        sa.Column(
            "item_id",
            sa.BigInteger,
            sa.ForeignKey("shared_irt.items.id", ondelete="CASCADE"),
            primary_key=True,
        ),
        ...
        sa.Column(
            ...
            comment="Min theta for info curve",
        ),
        sa.Column(
            "theta_max",
            sa.Float,
            server_default=sa.text("4.0"),
            comment="Max theta for info curve",
        ),
        sa.Column(
            "version",
            sa.Integer,
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            sa.Text,
            nullable=False,
            comment="Human-readable label (e.g., '2025-Q1')",
        ),
        sa.Column(
            "start_at",
            sa.TIMESTAMP(timezone=True),
    ...
    op.create_index(
        "idx_windows_times", "windows", ["start_at", "end_at"], schema="shared_irt"
    )
    op.create_index(
        "idx_windows_label", "windows", ["label"], unique=True, schema="shared_irt"
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "window_id",
            sa.BigInteger,
            sa.ForeignKey("shared_irt.windows.id", ondelete="CASCADE"),
            nullable=False,
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            sa.ForeignKey("shared_irt.items.id", ondelete="CASCADE"),
            nullable=True,
            comment="Item ID (NULL for test-level alerts)",
        ),
        sa.Column(
            "window_id",
            sa.BigInteger,
            sa.ForeignKey("shared_irt.windows.id", ondelete="CASCADE"),
            nullable=False,
            comment="Window where drift detected",
        ),
        sa.Column(
            "metric",
            sa.Text,
            nullable=False,
            comment="Metric: delta_b, delta_a, delta_c, dif_*, info_drop_*",
        ),
        sa.Column("value", sa.Float, comment="Observed metric value"),
        sa.Column("threshold", sa.Float, comment="Threshold exceeded"),
        sa.Column(
            "severity", sa.Text, nullable=False, comment="Severity: low, medium, high"
        ),
        sa.CheckConstraint(
            "severity IN ('low','medium','high')", name="da_severity_chk"
        ),
    ...
    op.create_index(
        "idx_drift_alerts_window", "drift_alerts", ["window_id"], schema="shared_irt"
    )
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            sa.Text,
            nullable=False,
            comment="Hashed user ID for privacy",
        ),
        ...
        sa.Column(
            "started_at", sa.TIMESTAMP(timezone=True), comment="When item was presented"
        ),
        sa.Column(
            "answered_at",
            sa.TIMESTAMP(timezone=True),
            server_default=sa.text("now()"),
            nullable=False,
            comment="When response submitted",
        ),
        ...
        sa.Column(
            ...
            postgresql.JSONB,
            server_default=sa.text("'{}'"),
            comment="Full response data",
        ),
        sa.Column("lang", sa.Text, nullable=False, comment="Response language"),
        sa.CheckConstraint(
            "lang IN ('en','ko','zh-Hans','zh-Hant')", name="ir_lang_chk"
        ),
        sa.Column(
            "extra",
            postgresql.JSONB,
...
def downgrade():
    ...
    op.drop_index(
        "idx_item_responses_user_hash", table_name="item_responses", schema="shared_irt"
    )
    op.drop_index(
        "idx_item_responses_session", table_name="item_responses", schema="shared_irt"
    )
    op.drop_index(
        "idx_item_responses_answered_at",
        table_name="item_responses",
        schema="shared_irt",
    )
    op.drop_index(
        "idx_item_responses_item", table_name="item_responses", schema="shared_irt"
    )
    op.drop_table("item_responses", schema="shared_irt")

    op.drop_index(
        "idx_drift_alerts_window", table_name="drift_alerts", schema="shared_irt"
    )
    op.drop_index(
        "idx_drift_alerts_severity_unresolved",
    ...
    op.drop_index(
        "idx_drift_alerts_item_window", table_name="drift_alerts", schema="shared_irt"
    )
    op.drop_table("drift_alerts", schema="shared_irt")

    op.drop_index(
        "idx_item_calibration_drift", table_name="item_calibration", schema="shared_irt"
    )
    op.drop_index(
        "idx_item_calibration_window",
        table_name="item_calibration",
        schema="shared_irt",
    )
    op.drop_index(
        "idx_item_calibration_item_window",
