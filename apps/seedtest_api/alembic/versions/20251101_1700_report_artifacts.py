...
def upgrade() -> None:
    ...
    if not _table_exists(conn, "report_artifacts"):
        op.create_table(
            ...
            sa.Column(
                "format", sa.Text, nullable=False, server_default="html"
            ),  # html, pdf
        ...
        # Indexes
        op.create_index(
            "ix_report_artifacts_user_week",
            "report_artifacts",
            ["user_id", "week_start"],
        )
        op.create_index(
            "ix_report_artifacts_generated_at", "report_artifacts", ["generated_at"]
        )
...
def downgrade() -> None:
    ...
    if _table_exists(conn, "report_artifacts"):
        ...
        op.drop_constraint(
            "uq_report_artifacts_user_week_format", "report_artifacts", type_="unique"
        )
