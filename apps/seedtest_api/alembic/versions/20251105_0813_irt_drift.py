...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('options_rich', postgresql.JSONB(astext_type=sa.Text()), nullable=True),
        sa.Column('answer_key', postgresql.JSONB(astext_type=sa.Text()), nullable=True),
        sa.Column('topic_tags', postgresql.ARRAY(sa.Text()), server_default='{}', nullable=True),
        sa.Column('subtopic_tags', postgresql.ARRAY(sa.Text()), server_default='{}', nullable=True),
        sa.Column('curriculum_tags', postgresql.ARRAY(sa.Text()), server_default='{}', nullable=True),
        ...
        sa.CheckConstraint("lang IN ('en','ko','zh-Hans','zh-Hant')", name='items_lang_check'),
    ...
    op.create_table(
        ...
        sa.Column('updated_at', sa.TIMESTAMP(timezone=True), server_default=sa.text('now()'), nullable=True),
        sa.CheckConstraint("model IN ('1PL','2PL','3PL')", name='item_parameters_model_check'),
        sa.ForeignKeyConstraint(['item_id'], ['shared_irt.items.id'], ondelete='CASCADE'),
    ...
    op.create_table(
        ...
        sa.Column('population_tags', postgresql.ARRAY(sa.Text()), server_default='{}', nullable=True),
        sa.Column('created_at', sa.TIMESTAMP(timezone=True), server_default=sa.text('now()'), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column('dif_metadata', postgresql.JSONB(astext_type=sa.Text()), server_default='{}', nullable=True),
        sa.Column('created_at', sa.TIMESTAMP(timezone=True), server_default=sa.text('now()'), nullable=True),
        sa.CheckConstraint("model IN ('1PL','2PL','3PL')", name='item_calibration_model_check'),
        sa.ForeignKeyConstraint(['item_id'], ['shared_irt.items.id'], ondelete='CASCADE'),
        sa.ForeignKeyConstraint(['window_id'], ['shared_irt.windows.id'], ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('id'),
        sa.UniqueConstraint('item_id', 'window_id', name='item_calibration_window_unique'),
        schema='shared_irt'
    )
    ...
    op.create_table(
        ...
        sa.Column('created_at', sa.TIMESTAMP(timezone=True), server_default=sa.text('now()'), nullable=True),
        sa.CheckConstraint("severity IN ('low','medium','high')", name='drift_alerts_severity_check'),
        sa.ForeignKeyConstraint(['item_id'], ['shared_irt.items.id'], ondelete='CASCADE'),
        sa.ForeignKeyConstraint(['window_id'], ['shared_irt.windows.id'], ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('id'),
        schema='shared_irt'
    ...
    op.create_table(
        ...
        sa.Column('response_payload', postgresql.JSONB(astext_type=sa.Text()), server_default='{}', nullable=True),
        sa.Column('lang', sa.Text(), nullable=False),
        sa.Column('extra', postgresql.JSONB(astext_type=sa.Text()), server_default='{}', nullable=True),
        sa.CheckConstraint("lang IN ('en','ko','zh-Hans','zh-Hant')", name='item_responses_lang_check'),
        sa.ForeignKeyConstraint(['item_id'], ['shared_irt.items.id'], ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('id'),
        schema='shared_irt'
    )
    ...
    # Performance Indexes
    # ============================================================================
    op.create_index('idx_item_responses_item', 'item_responses', ['item_id'], schema='shared_irt')
    op.create_index('idx_item_responses_window', 'item_responses', ['answered_at'], schema='shared_irt')
    op.create_index('idx_item_responses_org_user', 'item_responses', ['org_id', 'user_id_hash'], schema='shared_irt')
    op.create_index('idx_items_bank_lang', 'items', ['bank_id', 'lang'], schema='shared_irt')
    op.create_index(
    ...
    op.create_index('idx_item_calibration_item_window', 'item_calibration', ['item_id', 'window_id'], schema='shared_irt')
