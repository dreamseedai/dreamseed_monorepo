...
async def get_test_info_curve(
    ...
    """
    if len(item_ids) > 200:
        raise HTTPException(status_code=400, detail="Maximum 200 items per test")
...
async def get_global_stats(
    db: Connection = Depends(get_db), current_user=Depends(get_current_user)
):
    """
...
async def generate_monthly_report(
    ...
    download: bool = Query(
        True, description="Download file (True) or return path (False)"
    ),
    db: Connection = Depends(get_db),
    current_user=Depends(get_current_user),
):
    ...
    # Sanitize filename
    safe_label = "".join(
        c if c.isalnum() or c in ("-", "_") else "_" for c in window_label
    )
...
async def health_check(db: Connection = Depends(get_db)):
    ...
    try:
        ...
        return {"status": "healthy", "database": "connected", "total_items": result}
    except Exception as e:
        raise HTTPException(
            status_code=503, detail=f"IRT analytics system unhealthy: {str(e)}"
        )
