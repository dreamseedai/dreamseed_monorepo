...
os.environ.setdefault("DATABASE_URL", "postgresql://postgres:postgres@127.0.0.1:5432/dreamseed")
...
def test_student_create_and_read_own_session():
    ...
    assert session_data["user_id"] == "dev-user", f"Expected user_id='dev-user', got {session_data['user_id']}"
    assert session_data["org_id"] == 1, f"Expected org_id=1, got {session_data['org_id']}"
...
def test_student_cannot_read_other_user_session():
    ...
    response = client.post("/api/seedtest/sessions/", json=payload)
    print(f"POST /api/seedtest/sessions/ with user_id='other_user' -> {response.status_code}")
