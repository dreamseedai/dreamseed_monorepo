...
class EnvSettingsSource(PydanticBaseEnvSettingsSource):
    ...
    @staticmethod
    def next_field(
        field: FieldInfo | Any | None, key: str, case_sensitive: bool | None = None
    ) -> FieldInfo | None:
        """
...
class CliSettingsSource(EnvSettingsSource, Generic[T]):
    ...
    @overload
    def __call__(
        self, *, parsed_args: Namespace | dict[str, list[str] | str]
    ) -> CliSettingsSource[T]:
        """
    ...
    def _load_env_vars(
        ...
        parsed_args = {
            ...
            if not key.endswith(":subcommand")
        }
        ...
        try:
            ...
            if (
                ...
                or not any(
                    ...
                    and get_origin(type_) not in (dict, Mapping)
                )
            ):
        ...
        for field_name, field_info in fields.items():
            if _CliSubCommand in field_info.metadata:
                if not field_info.is_required():
                    ...
                    raise SettingsError(
                        f"subcommand argument {model.__name__}.{field_name} has an alias"
                    )
                else:
                    field_types = [
                        type_
                    ...
                    if len(field_types) != 1:
                        raise SettingsError(
                            f"subcommand argument {model.__name__}.{field_name} has multiple types"
                        )
                    elif not (
                        is_model_class(field_types[0])
                        or is_pydantic_dataclass(field_types[0])
                    ):
                        raise SettingsError(
                ...
                if not field_info.is_required():
                    raise SettingsError(
                        f"positional argument {model.__name__}.{field_name} has a default value"
                    )
                elif any((field_info.alias, field_info.validation_alias)):
                    raise SettingsError(
                        f"positional argument {model.__name__}.{field_name} has an alias"
                    )
    ...
    def _connect_root_parser(
        ...
        self._parse_args = self._connect_parser_method(
            parse_args_method, "parsed_args_method"
        )
        ...
        for field_name, field_info in self._sort_arg_fields(model):
            ...
            if _CliSubCommand in field_info.metadata:
                if subparsers is None:
                    ...
                    self._cli_subcommands[f"{arg_prefix}:subcommand"].append(
                        f"{arg_prefix}{field_name}"
                    )
                ...
                resolved_names, is_alias_path_only = self._get_resolved_names(
                    field_name, field_info, alias_path_args
                )
                arg_flag: str = "--"
                kwargs: dict[str, Any] = {}
                ...
                kwargs["required"] = (
                    self.cli_enforce_required and field_info.is_required()
                )
                ...
                arg_names = self._get_arg_names(
                    arg_prefix, subcommand_prefix, alias_prefixes, resolved_names
                )
                ...
                if sub_models and kwargs.get("action") != "append":
                    ...
                    self._add_argument(
                        group,
                        *(f"{arg_flag[:len(name)]}{name}" for name in arg_names),
                        **kwargs,
                    )
                else:
                    added_args += list(arg_names)
    ...
    def _add_parser_submodels(
        ...
        if not self.cli_avoid_json:
            ...
            self._add_argument(
                model_group, *(f"{arg_flag}{name}" for name in arg_names), **kwargs
            )
    ...
    def _add_parser_alias_paths(
        ...
        if alias_path_args:
            ...
            if group is not None:
                context = (
                    self._add_argument_group(parser, **group)
                    if isinstance(group, dict)
                    else group
                )
...
class AzureKeyVaultMapping(Mapping[str, Optional[str]]):
    ...
    def __init__(
        ...
        self._secret_names: list[str] = [
            secret.name for secret in self._secret_client.list_properties_of_secrets()
        ]
...
class AzureKeyVaultSettingsSource(EnvSettingsSource):
    ...
    def __repr__(self) -> str:
        return (
            f"AzureKeyVaultSettingsSource(url={self._url!r}, "
