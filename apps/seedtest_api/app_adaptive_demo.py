...
DEMO_POOL: List[Dict] = [
    {"question_id": 1, "discrimination": 1.2, "difficulty": -0.5, "guessing": 0.2, "topic_id": 10, "tags": ["algebra"]},
    {"question_id": 2, "discrimination": 1.0, "difficulty": 0.0, "guessing": 0.2, "topic_id": 10, "tags": ["algebra", "functions"]},
    {"question_id": 3, "discrimination": 1.4, "difficulty": 0.5, "guessing": 0.2, "topic_id": 20, "tags": ["geometry"]},
    {"question_id": 4, "discrimination": 0.9, "difficulty": 1.0, "guessing": 0.2, "topic_id": 20, "tags": ["geometry"]},
]
...
def start_session(req: StartRequest) -> StartResponse:
    ...
    try:
        ...
        if settings.CAT_START_RANDOMIZED:
            ...
            start_items = [it for it in DEMO_POOL if lo <= float(it.get("difficulty", 0.0)) <= hi] or DEMO_POOL
        item, info, _ = select_next_with_constraints(
            ...
            keymap=km,
            prefilter=pre,
            top_n=max(int(settings.CAT_START_TOP_N if settings.CAT_START_RANDOMIZED else settings.CAT_TOP_N), 1),
    ...
    return StartResponse(session_id=sid, theta=theta, next_item_id=next_id, next_item_info=info, policy=pol, started_at=started_at)
...
def answer(session_id: str, req: AnswerRequest) -> AnswerResponse:
    ...
    theta = float(s["theta"]) ; test_info = float(s["test_info"]) ; used_ids: List[int] = list(s["used_ids"])  # copy
    ...
    a = float(item["discrimination"]) ; b = float(item["difficulty"]) ; c = float(item.get("guessing", 0.0))
    ...
    sem_threshold = float(settings.CAT_SEM_THRESHOLD) if settings.CAT_SEM_THRESHOLD is not None else None
    max_items = int(settings.CAT_MAX_ITEMS) if settings.CAT_MAX_ITEMS is not None else None
    min_items = int(settings.CAT_MIN_ITEMS) if settings.CAT_MIN_ITEMS is not None else 1
    max_time = int(settings.CAT_MAX_TIME_SECONDS) if settings.CAT_MAX_TIME_SECONDS is not None else None
    ...
    if cooldown is not None:
        ...
        if last_answer_at is None:
            ...
            s.update({"theta": theta_new, "test_info": test_info_new, "used_ids": used_ids, "last_answer_at": now_ts})
            return AnswerResponse(theta=theta_new, se=se, next_item_id=None, reason="cooldown", terminated=False, items_answered=items_answered, elapsed_seconds=elapsed, cooldown_remaining=float(cooldown))
        else:
            ...
            if since < float(cooldown):
                ...
                return AnswerResponse(theta=theta_new, se=se, next_item_id=None, reason="cooldown", terminated=False, items_answered=items_answered, elapsed_seconds=elapsed, cooldown_remaining=float(cooldown - since))

    # Manual termination takes precedence if allowed
    if req.finish_now:
        s.update({"theta": theta_new, "test_info": test_info_new, "used_ids": used_ids})
        return AnswerResponse(theta=theta_new, se=se, next_item_id=None, reason="manual_finish", terminated=True, items_answered=items_answered, elapsed_seconds=elapsed)

    # Fixed-length mode: stop when items reach max (if set)
    if settings.CAT_MODE == "FIXED" and max_items is not None and items_answered >= max_items:
        s.update({"theta": theta_new, "test_info": test_info_new, "used_ids": used_ids})
        return AnswerResponse(theta=theta_new, se=se, next_item_id=None, reason="max_items", terminated=True, items_answered=items_answered, elapsed_seconds=elapsed)

    # Time limit
    if max_time is not None and elapsed is not None and elapsed >= max_time:
        s.update({"theta": theta_new, "test_info": test_info_new, "used_ids": used_ids})
        return AnswerResponse(theta=theta_new, se=se, next_item_id=None, reason="time_limit", terminated=True, items_answered=items_answered, elapsed_seconds=elapsed)

    # Variable-length precision stop (after at least min_items); gate by minimum test time if provided
    if settings.CAT_MODE == "VARIABLE" and sem_threshold is not None and items_answered >= max(min_items, 1) and se <= sem_threshold:
        ...
        if min_time is not None and elapsed is not None and elapsed < float(min_time):
            ...
            s.update({"theta": theta_new, "test_info": test_info_new, "used_ids": used_ids})
            return AnswerResponse(theta=theta_new, se=se, next_item_id=None, reason="sem_threshold", terminated=True, items_answered=items_answered, elapsed_seconds=elapsed)

    # Choose next using constrained selector
    try:
        km = KeyMap(id="question_id", a="discrimination", b="difficulty", c="guessing")
        pre = _make_prefilter(topic_ids=s["topic_ids"], tags_any=s["tags_any"]) if (s.get("topic_ids") or s.get("tags_any")) else None
        bp = None
        if settings.CAT_BLUEPRINT:
            import json
            try:
