...
from sqlalchemy import String, Date, DateTime, Float, Integer, Boolean, JSON, Index, UniqueConstraint, text
...
class StudentMood(Base):
    ...
    mood: Mapped[str] = mapped_column(String(8), nullable=False)  # 'happy' | 'neutral' | 'sad'
    note: Mapped[Optional[str]] = mapped_column(String(512), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=text("now()"))
    
    __table_args__ = (
        UniqueConstraint('tenant_id', 'student_id', 'day', name='uq_mood_tenant_student_day'),
...
class StudentDailyLog(Base):
    ...
    student_id: Mapped[str] = mapped_column(String, index=True, nullable=False)
    day: Mapped[date] = mapped_column(Date, index=True, nullable=False)
    study_minutes: Mapped[int] = mapped_column(Integer, server_default='0', nullable=False)
    tasks_done: Mapped[int] = mapped_column(Integer, server_default='0', nullable=False)
    theta_delta: Mapped[float] = mapped_column(Float, server_default='0.0', nullable=False)
    reflections: Mapped[Optional[str]] = mapped_column(String(1000), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=text("now()"))
    
    __table_args__ = (
        UniqueConstraint('tenant_id', 'student_id', 'day', name='uq_log_tenant_student_day'),
...
class StudentGoal(Base):
    ...
    __table_args__ = (
        Index('ix_goal_tenant_student_created', 'tenant_id', 'student_id', 'created_at'),
    )
...
class StudentAIMessage(Base):
    ...
    tone: Mapped[str] = mapped_column(String(24), server_default="'warm'", nullable=False)  # 'warm' | 'gentle' | 'energetic'
    meta: Mapped[dict] = mapped_column(JSON, server_default='{}', nullable=False)  # Metadata (e.g., generation context)
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=text("now()"))
    
    __table_args__ = (
        UniqueConstraint('tenant_id', 'student_id', 'day', name='uq_msg_tenant_student_day'),
