...
# Log format with upstream status for debugging
log_format ds_combined '$remote_addr - $remote_user [$time_local] '
...
server {
    ...
    # (Optional) Enable brotli if module is available
    # brotli on;
    # brotli_comp_level 5;
    # brotli_types text/plain text/css application/javascript application/json application/ld+json application/xml image/svg+xml;
    ...
    # staging=off, prod=on (단계적 적용)
    set $hsts ${HSTS_ENABLED};
    if ($hsts = on) { 
        add_header Strict-Transport-Security "max-age=31536000; includeSubDomains" always; 
    }

    # Staging only: prevent indexing and add basic auth
    set $is_staging ${SERVER_NAME};
    ...
    location /api/ {
        ...
        # CORS는 앱 레이어에서 처리 (nginx에서 임의 CORS 헤더 강제 금지)
    }
