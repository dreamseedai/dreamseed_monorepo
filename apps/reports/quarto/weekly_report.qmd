...
```{r prophet-forecast}
#| fig-cap: "Prophet Forecast with 80%/95% Intervals and Anomaly Detection"
#| fig-width: 10
#| fig-height: 6
...
if (!is.null(data$prophet_forecast) && length(data$prophet_forecast) > 0) {
  ...
  # Extract insample + forecast
  insample <- pf$insample
  forecast <- pf$forecast
  anomalies <- pf$anomalies
  model_meta <- pf$model_meta
  
  # Combine insample and forecast for plotting
  plot_data <- NULL
  if (!is.null(insample) && length(insample) > 0) {
    insample_df <- bind_rows(insample) %>% 
      mutate(ds = as.Date(ds), type = "Historical")
    plot_data <- insample_df
  }
  if (!is.null(forecast) && length(forecast) > 0) {
    forecast_df <- bind_rows(forecast) %>% 
      mutate(ds = as.Date(ds), type = "Forecast")
    plot_data <- if (is.null(plot_data)) forecast_df else bind_rows(plot_data, forecast_df)
  }
  
  if (!is.null(plot_data) && nrow(plot_data) > 0) {
    # Base plot with confidence bands
    p <- ggplot(plot_data, aes(x = ds)) +
      ...
      # Forecast line
      geom_line(
        data = plot_data %>% filter(type == "Forecast"),
        aes(y = yhat),
        color = "steelblue", size = 1, linetype = "dashed"
      ) +
      # Historical line
      geom_line(
        data = plot_data %>% filter(type == "Historical"),
        aes(y = yhat),
        color = "darkblue", size = 1
      ) +
      # Actual observations
      geom_point(
        data = plot_data %>% filter(!is.na(y)),
        aes(y = y),
    ...
    if (!is.null(anomalies) && length(anomalies) > 0) {
      anom_df <- bind_rows(anomalies) %>% 
        mutate(ds = as.Date(ds)) %>%
        filter(!is.na(anomaly_score))
      
      if (nrow(anom_df) > 0) {
        # Join with plot_data to get y values
        anom_df <- anom_df %>%
          left_join(plot_data %>% select(ds, y), by = "ds")
        
        p <- p + geom_point(
          data = anom_df,
          aes(y = y, size = abs(anomaly_score)),
          color = "red", alpha = 0.7, shape = 17
        ) +
    ...
    p <- p +
      theme_minimal() +
      labs(
        title = "Prophet Time Series Forecast (I_t)",
        subtitle = ifelse(
          !is.null(model_meta$changepoints),
          paste0("Changepoints detected: ", model_meta$changepoints),
          "4-week ahead forecast with 95% prediction intervals"
        ),
        x = "Date",
        y = "Improvement Index (I_t)"
      ) +
      theme(
        plot.title = element_text(size = 14, face = "bold"),
        plot.subtitle = element_text(size = 11, color = "gray40"),
    ...
    # Display fit metrics if available
    if (!is.null(model_meta$fit_metrics)) {
      fm <- model_meta$fit_metrics
      cat("\n**Model Fit Metrics**:\n")
      cat(paste0("- RMSE: ", round(fm$rmse, 4), "\n"))
      cat(paste0("- MAE: ", round(fm$mae, 4), "\n"))
      if (!is.null(model_meta$horizon_weeks)) {
        cat(paste0("- Forecast Horizon: ", model_meta$horizon_weeks, " weeks\n"))
      }
    }
  } else {
    cat("No forecast data available for plotting.\n")
  }
  
  # Anomaly table
  if (!is.null(anomalies) && length(anomalies) > 0) {
    an_df <- bind_rows(anomalies) %>%
      mutate(ds = as.Date(ds)) %>%
      arrange(desc(abs(anomaly_score))) %>%
      select(ds, y, yhat, residual, anomaly_score) %>%
      head(10)
    ...
    if (nrow(an_df) > 0) {
      ...
      knitr::kable(
        an_df,
        col.names = c("Date", "Actual", "Predicted", "Residual", "Anomaly Score"),
        caption = "Top Anomalies (by |z-score|)",
        digits = 3
      )
      cat("\n**Note**: Anomaly scores represent standardized residuals (z-scores). Values beyond ±2.5 indicate unusual deviations.\n")
    }
  }
} else {
  cat("Prophet forecast not available for this period. Run `forecast_prophet.py` to generate predictions.\n")
}
```

---

## Survival Analysis: Churn Risk Assessment

```{r survival-risk-gauge}
#| fig-cap: "14-Day Inactivity Risk Gauge"
#| fig-width: 8
#| fig-height: 4
...
if (!is.null(sr)) {
  # Extract risk score (S from weekly_kpi or direct from survival_risk)
  risk_score <- if (!is.null(sr$churn_risk)) {
    sr$churn_risk
  ...
  if (!is.na(risk_score)) {
    ...
    # Gauge visualization
    gauge_df <- data.frame(
      Category = c("Safe", "Risk"),
      Value = c(1 - risk_score, risk_score)
    )
    
    ggplot(gauge_df, aes(x = 2, y = Value, fill = Category)) +
      geom_col(position = "stack", color = "white", size = 1) +
      coord_polar(theta = "y", start = 0) +
      xlim(c(0.5, 2.5)) +
      scale_fill_manual(
        values = c("Safe" = "#2ECC71", "Risk" = risk_color),
        name = NULL
      ) +
      theme_void() +
      theme(
      ...
      labs(
        title = "14-Day Churn Risk",
        subtitle = paste0(risk_level, ": ", round(risk_score * 100, 1), "%")
      ) +
      annotate(
        "text",
        x = 0.5,
        y = 0,
        label = paste0(round(risk_score * 100, 1), "%"),
        size = 8,
        fontface = "bold",
        color = risk_color
      )
    ...
    cat("\n\n")
    cat(paste0("**Risk Assessment**: ", risk_level, " (", round(risk_score * 100, 1), "% probability)\n\n"))
    
    # Risk interpretation
    if (risk_score < 0.3) {
      cat("✅ Your engagement pattern shows consistent activity. Keep up the good work!\n")
    } else if (risk_score < 0.7) {
      cat("⚠️ Your activity has decreased recently. Consider setting a regular study schedule.\n")
    } else {
      cat("🚨 High inactivity risk detected. We recommend reconnecting with your learning goals.\n")
    }
  } else {
    cat("Risk score not available.\n")
  }
  
  # Display hazard ratios from survival model
  if (!is.null(sr$fit_meta)) {
    fm <- sr$fit_meta
    
    if (!is.null(fm$coefficients)) {
      cat("\n\n### Survival Model Coefficients\n\n")
      coef_df <- data.frame(
        Covariate = names(fm$coefficients),
        Coefficient = unlist(fm$coefficients),
        stringsAsFactors = FALSE
      )
      
      # Calculate hazard ratios (exp(coef))
      coef_df$HazardRatio <- exp(coef_df$Coefficient)
      coef_df$Interpretation <- ifelse(
        coef_df$HazardRatio > 1,
        "Increases risk",
        "Decreases risk"
      )
      
      knitr::kable(
        coef_df %>% select(Covariate, Coefficient, HazardRatio, Interpretation),
        col.names = c("Covariate", "β Coefficient", "Hazard Ratio", "Effect"),
        caption = "Cox Proportional Hazards Model Results",
        digits = 3
      )
      
      cat("\n**Interpretation**: Hazard ratios > 1 indicate increased churn risk; < 1 indicate protective factors.\n")
    }
    
    if (!is.null(fm$concordance)) {
      cat(paste0("\n**Model Performance**: C-index = ", round(fm$concordance, 3), " (discrimination ability)\n"))
    }
  }
} else {
  cat("Survival risk data not available for this period. Run `fit_survival_churn.py` to generate risk scores.\n")
}
```
...
```{r survival-curve}
#| fig-cap: "Survival Probability Over Time"
#| fig-width: 8
#| fig-height: 5

if (!is.null(sr) && !is.null(sr$fit_meta) && !is.null(sr$fit_meta$survival_curve)) {
  sc <- sr$fit_meta$survival_curve
  ...
  if (length(sc) > 0) {
    ...
    if (nrow(sc_df) > 0) {
      # Plot survival curve
      ggplot(sc_df, aes(x = time, y = surv)) +
        ...
        theme_minimal() +
        labs(
          title = "Population Survival Curve",
      ...
      if (length(median_surv) > 0 && !is.na(median_surv)) {
        cat(paste0("\n**Median Survival Time**: ", round(median_surv, 1), " days (50% of users remain active)\n"))
      }
    } else {
      cat("Survival curve data is empty.\n")
    }
  } else {
    cat("No survival curve data available.\n")
  }
} else {
  cat("Survival curve not available. Ensure `fit_survival_churn.py` has been run with sufficient data.\n")
}
```
