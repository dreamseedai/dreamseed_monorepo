...
.venv/bin/uvicorn app.main:app --host 127.0.0.1 --port 8011 --log-level warning &
PID=$!
sleep 1

# 헬스
curl -sf http://127.0.0.1:8011/__ok >/dev/null && echo "[OK] __ok"

# 회원가입(있으면 무시)
curl -s -X POST http://127.0.0.1:8011/auth/register \
  -H "Content-Type: application/json" \
  -d "{\"email\":\"$EMAIL\",\"password\":\"$PASS\"}" >/dev/null || true

# 로그인 → AT 추출 (jq 없이 파이썬 사용)
AT=$(curl -s -X POST http://127.0.0.1:8011/auth/login \
  -H "Content-Type: application/json" \
...
# me 확인
curl -s http://127.0.0.1:8011/auth/me -H "Authorization: Bearer $AT" && echo

# 콘텐츠 생성 & 조회
curl -s -X POST http://127.0.0.1:8011/content/ \
...
curl -s http://127.0.0.1:8011/content/ -H "Authorization: Bearer $AT" && echo
