#!/usr/bin/env python3
"""
Working MySQL Dump Parser for mpcstudy.com data
...
class WorkingMySQLParser:
    ...
    def _extract_insert_statements(self) -> List[str]:
        """
        Extract INSERT statements by reading line by line and handling multi-line statements
        """
        insert_statements = []
        current_statement = ""
        in_insert = False
        ...
        with open(self.dump_file_path, 'r', encoding='utf-8', errors='ignore') as f:
            for line_num, line in enumerate(f, 1):
                ...
                # Progress indicator
                if line_num % 10000 == 0:
                    logging.info(f"Processed {line_num} lines, found {len(insert_statements)} INSERT statements")
        
        logging.info(f"Found {len(insert_statements)} INSERT statements")
        return insert_statements

    def _parse_insert_statement(self, insert_statement: str) -> List[ProblemData]:
        """
        Parse a single INSERT statement into ProblemData objects
        """
        problems = []
        ...
        if not values_match:
            logging.warning(f"Could not extract VALUES from: {insert_statement[:100]}...")
            return problems
...
if __name__ == "__main__":
    ...
    parser = WorkingMySQLParser(dump_file)
