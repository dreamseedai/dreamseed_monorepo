...
@app.get("/api/seedtest/topics")
def get_topics_compat(subject: str | None = None, db: Session = Depends(get_db)):
    """토픽 목록 조회 (호환성 엔드포인트)"""
    # questions.get_topics와 동일한 로직
    query = db.query(Problem.mysql_metadata['que_en_title'].astext.label('topic')).distinct()
    
    if subject:
        query = query.filter(Problem.mysql_metadata['que_class'].astext == subject)
    
    topics = [row.topic for row in query.all() if row.topic]
    return topics
