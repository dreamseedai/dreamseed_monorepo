...
# 테스트 URL
http://localhost:3000/questions/1000/edit
...
완벽합니다! 이제 테스트해볼 수 있습니다. TinyMCE와 Wiris CDN이 제대로 로드되는지 확인 요약을 드리겠습니다:
...
포트: 9001
엔드포인트: http://localhost:9001/api/seedtest/problems/{id}
...
포트: 3030 (기존 3000 아님!)
URL: http://localhost:3030/questions/1000/edit

3️⃣ TinyMCE 4 + Wiris 설정 확인
로컬 TinyMCE:

tinymceScriptSrc="/tinymce/tinymce.min.js"  // ✅ 존재
...
CDN: jsDelivr ✅
버전: @wiris/mathtype-tinymce4 (TinyMCE 4 전용)

4️⃣ 테스트 방법

# 브라우저에서 접속:
http://localhost:3030/questions/1000/edit

# 확인 사항:
1. 페이지가 로드되는지
2. TinyMCE 에디터가 나타나는지
3. 수식 버튼(Wiris)이 툴바에 있는지
4. 문제 1000번 데이터가 로드되는지
...
// Wiris 로드 확인
console.log('WIRIS loaded:', !!window.WirisPlugin);

6️⃣ Network 탭 확인
다음 리소스가 성공(200)해야 합니다:

GET /tinymce/tinymce.min.js → 200 OK
GET https://cdn.jsdelivr.net/npm/@wiris/mathtype-tinymce4/plugin.min.js → 200 OK
GET http://localhost:9001/api/seedtest/questions/1000 → 200 OK (또는 401 인증 필요)
포트 3030으로 접속하세요! (3000이 아닙니다)
_______________________________________________________________
...
1. Next.js 빌드 ✅
    TypeScript 타입 에러 수정 (Question.id: string → number)
    프로덕션 빌드 성공

2. PM2 설정 ✅
    PM2 글로벌 설치
    admin-front 앱 등록 및 실행
...
4. 프로덕션 사이트 연결 ✅
    gpt-access.html 수정
    "문제 관리" 버튼 → /questions 링크

5. 환경 변수 업데이트 ✅
    API URL: https://dreamseedai.com/api/seedtest

🌐 접속 방법:

프로덕션 URL:

https://dreamseedai.com/questions

사이드바에서:
    https://dreamseedai.com/admin/gpt-access.html 접속
    왼쪽 사이드바 → "문제 관리" 클릭
    TinyMCE 에디터가 새 탭에서 열림
...
# 로그 확인
pm2 logs admin-front

# 재시작
pm2 restart admin-front

# 중지
pm2 stop admin-front
...
console.log('TinyMCE:', window.tinymce?.majorVersion);
console.log('Wiris:', !!window.WirisPlugin);

Network 탭에서 확인:

✅ /tinymce/tinymce.min.js (로컬)
✅ https://cdn.jsdelivr.net/npm/@wiris/mathtype-tinymce4/plugin.min.js (CDN)
모든 준비 완료! 🚀
______________________________________________________________
