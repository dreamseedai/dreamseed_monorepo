...
export async function listQuestions(filter: QuestionFilter & { cursor?: string }): Promise<{ results: Question[]; total: number; nextCursor?: string }> {
  ...
  const raw = await http<{ questions?: Question[]; results?: Question[]; data?: Record<string, Question>; total?: number; total_count?: number; next_cursor_opaque?: string }>(`${API_URL}/questions?${params.toString()}`);
  
  // Handle multiple response formats: questions[], results[], or data{}
  ...
  const total = raw.total_count ?? raw.total ?? 0;
