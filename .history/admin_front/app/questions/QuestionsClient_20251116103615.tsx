  ...
  return (
    <section className="space-y-4">
      ...
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-semibold" data-testid="questions-heading">문항은행 ({total.toLocaleString()})</h1>
        <Link href="/questions/new" className="rounded bg-blue-600 px-4 py-2 text-white text-sm font-semibold">➕ 신규 문항 추가</Link>
      </div>
        ...
        <table className="min-w-full text-sm">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-3 py-2 w-28">Grade</th>
              ...
              <th className="px-3 py-2">
                <button
                  ...
                  onClick={() => setFilter(f => ({ ...f, sortBy: 'created_at', order: f.order === 'desc' ? 'asc' : 'desc', page: 1 }))}
                  title="생성일 정렬 토글"
                >
                  Created
                  <span className="text-xs text-gray-500">{filter.sortBy === 'created_at' ? (filter.order === 'desc' ? '▼' : '▲') : ''}</span>
                </button>
              </th>
              <th className="px-3 py-2">
                <button
          ...
          <tbody>
            {items.map((q) => (
              <tr key={q.id} className="border-t">
                ...
                <td className="px-3 py-2 text-center text-xs">{(q as any).que_class || '-'}</td>
                <td className="px-3 py-2 text-center text-xs">{(q as any).que_grade || '-'}</td>
                ...
                <td className="px-3 py-2 whitespace-nowrap text-xs">{fmtUpdatedAt(q, 'created')}</td>
                <td className="px-3 py-2 whitespace-nowrap text-xs">{fmtUpdatedAt(q)}</td>
                <td className="px-3 py-2">
                  <span className={
                ...
                <td className="px-3 py-2 text-center">
                  <button
                    ...
                    className={`${meta?.legacy_readonly_enabled ? 'text-gray-400 cursor-not-allowed' : 'text-red-600 hover:underline cursor-pointer'}`}
