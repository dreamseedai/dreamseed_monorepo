...
const API_PREFIX = process.env.NEXT_PUBLIC_API_PREFIX || '/api/seedtest';
const UPLOAD_MODE = process.env.NEXT_PUBLIC_UPLOAD_MODE || '';

// Lightweight ETag store (per-session)
const _etagById = new Map<string, string>();

function _setEtag(id: string, etag: string | null) {
  if (!id) return;
  if (etag && etag.trim()) _etagById.set(String(id), etag.trim());
}
function _getEtag(id: string): string | undefined {
  return _etagById.get(String(id));
}
function _clearEtag(id: string) {
  _etagById.delete(String(id));
}
...
export async function listQuestions(filter: QuestionFilter & { cursor?: string }): Promise<{ results: Question[]; total: number; nextCursor?: string }> {
  ...
  const raw = await http<{ results: Question[]; total: number; next_cursor_opaque?: string }>(`${API_PREFIX}/questions?${params.toString()}`);
  return { results: raw.results, total: raw.total, nextCursor: raw.next_cursor_opaque };
}
...
export async function getTopics(subject?: string): Promise<string[]> {
  ...
  const res = await fetch(`${API_PREFIX}/topics${sp.toString() ? `?${sp.toString()}` : ''}`, { cache: 'no-store' });
    if (res.ok) {
  ...
  try {
  const data = await http<any[]>(`${API_PREFIX}/questions/topics`);
    return mk(data);
  } catch {
    return [];
  }
...
export async function getQuestion(id: string): Promise<Question | null> {
  try {
  const res = await fetch(`${API_PREFIX}/questions/${id}`, { cache: 'no-store' });
    if (!res.ok) {
    ...
    _setEtag(id, res.headers.get('etag'));
...
export async function getQuestionWithSource(id: string): Promise<{ question: Question | null; source?: string }> {
  try {
  const res = await fetch(`${API_PREFIX}/questions/${id}`, { cache: 'no-store' });
    if (!res.ok) {
...
export async function createQuestion(input: QuestionInput): Promise<Question> {
  ...
  const res = await fetch(`${API_PREFIX}/questions`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
  ...
  if (q?.id && etag) _setEtag(String(q.id), etag);
  return q;
}
...
export async function updateQuestion(id: string, input: QuestionInput): Promise<Question | null> {
  try {
    ...
    const etag = _getEtag(id);
  const res = await fetch(`${API_PREFIX}/questions/${id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        'Idempotency-Key': idem,
...
export async function uploadImage(file: File): Promise<{ url: string; filename: string; content_type: string; size: number }> {
  ...
  if (UPLOAD_MODE.toLowerCase() === 'presigned') {
    try {
  const presignRes = await fetch(`${API_PREFIX}/uploads/images/presign`, {
        method: 'POST',
  ...
  const form = new FormData();
  form.append('file', file);
  const res = await fetch(`${API_PREFIX}/uploads/images`, {
    method: 'POST',
    body: form,
  });
  if (!res.ok) {
