...
console.log('[questions.ts] API_URL =', API_URL);
console.log('[questions.ts] NEXT_PUBLIC_API_URL =', process.env.NEXT_PUBLIC_API_URL);
...
export async function listQuestions(filter: QuestionFilter & { cursor?: string }): Promise<{ results: Question[]; total: number; nextCursor?: string }> {
  ...
  const raw = await http<{ results?: Question[]; data?: Record<string, Question>; total: number; next_cursor_opaque?: string }>(`${API_URL}/questions?${params.toString()}`);
  
  // Handle both array (results) and object (data) response formats
  ...
  return { results, total: raw.total, nextCursor: raw.next_cursor_opaque };
}
