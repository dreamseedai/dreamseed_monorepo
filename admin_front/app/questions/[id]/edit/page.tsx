...
export default function EditQuestionPage({ params }: { params: { id: string } }) {
  ...
  if (loading) {
    return (
      <section className="p-4 text-sm text-gray-600 dark:text-gray-400">
        로딩중...
      </section>
    );
  }
  ...
  if (error && !q) {
    return (
      <section className="p-4 text-sm text-red-600 dark:text-red-400">
        {error}
      </section>
    );
  }
  ...
  return (
    <section className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => !readOnly && setConfirmOpen(true)} className={`text-sm ${readOnly ? 'text-gray-400 dark:text-gray-500 cursor-not-allowed' : 'text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300'}`} title={readOnly ? '레거시 모드에서는 삭제할 수 없습니다.' : undefined}>삭제</button>
      </div>
      {/* Header info box (image preview removed as requested) */}
      <div className="rounded border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-3 text-sm grid grid-cols-1 md:grid-cols-3 gap-3 shadow-sm">
        <div>
          <div className="text-gray-500 dark:text-gray-400 text-xs">문항 ID</div>
          <div className="font-medium text-gray-900 dark:text-gray-100">{q.id}</div>
        </div>
        <div>
          <div className="text-gray-500 dark:text-gray-400 text-xs">등록일</div>
          <div className="font-medium text-gray-900 dark:text-gray-100">{q.created_at ? new Date(q.created_at).toLocaleString() : '-'}</div>
        </div>
        <div>
          <div className="text-gray-500 dark:text-gray-400 text-xs">수정일</div>
          <div className="font-medium text-gray-900 dark:text-gray-100">{q.updated_at ? new Date(q.updated_at).toLocaleString() : '-'}</div>
        </div>
      </div>
      ...
      <h1 className="text-xl font-semibold text-gray-900 dark:text-gray-100">문항 편집</h1>
      {readOnly && (
        <div className="rounded-md border border-yellow-300 dark:border-yellow-600 bg-yellow-50 dark:bg-yellow-900/20 text-yellow-900 dark:text-yellow-200 px-3 py-2 text-sm">
      ...
      {stale && (
        <div className="rounded border border-amber-300 dark:border-amber-600 bg-amber-50 dark:bg-amber-900/20 text-amber-900 dark:text-amber-200 p-3 text-sm">
          <div className="flex items-center justify-between gap-2">
            <span>{stale}</span>
            <button
              className="px-2 py-1 text-xs rounded bg-amber-600 dark:bg-amber-500 text-white hover:bg-amber-700 dark:hover:bg-amber-600 transition-colors"
              onClick={async () => {
                try {
