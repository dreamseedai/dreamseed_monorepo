...
export function QuestionForm({ initial, onSubmit, disabled }: { initial?: QuestionInput; onSubmit: (q: QuestionInput) => void; disabled?: boolean }) {
  ...
  const errors = useMemo(() => {
    const errs: string[] = [];
    const stem = form.stem || "";
    console.log('[QuestionForm DEBUG] stem:', stem, 'length:', stem.length, 'trimmed:', stem.trim().length);
    if (!stem.trim()) errs.push("문항 내용을 입력하세요.");
    ...
    console.log('[QuestionForm DEBUG] options:', form.options, 'trimmed:', trimmed);
    ...
    console.log('[QuestionForm DEBUG] answer:', form.answer, 'options length:', trimmed.length);
    ...
    console.log('[QuestionForm DEBUG] total errors:', errs);
    return errs;
  ...
  const refreshTopics = async () => {
    ...
    try {
      ...
      console.warn('토픽 로드 실패, 기본 토픽 사용', e);
      // 기본 토픽 목록 사용
      setTopics([
        { id: 1, name: '수학', org_id: null },
        { id: 2, name: '물리', org_id: null },
        { id: 3, name: '화학', org_id: null },
        { id: 4, name: '생명과학', org_id: null },
        { id: 5, name: '지구과학', org_id: null },
      ]);
    } finally {
  ...
  return (
    <form className="space-y-6 bg-white dark:bg-gray-900 transition-colors" onSubmit={submit} onKeyDown={(e) => {
      ...
      {disabled && (
        <div className="rounded-md border border-yellow-300 dark:border-yellow-600 bg-yellow-50 dark:bg-yellow-900/20 text-yellow-900 dark:text-yellow-200 px-3 py-2 text-sm">
      ...
      <div className="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
        <table className="min-w-full text-sm">
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
            {/* 1) 메타 */}
            <tr>
              <th className="w-36 bg-gray-50 dark:bg-gray-800 text-left px-3 py-2 text-gray-600 dark:text-gray-300 align-top font-semibold">메타</th>
              <td className="px-3 py-2 bg-white dark:bg-gray-900">
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  <div>
                    <label className="block text-xs text-gray-700 dark:text-gray-300 font-medium">난이도</label>
                    <select
                      className="mt-1 w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 p-2 text-sm focus:border-blue-500 dark:focus:border-blue-400 focus:outline-none"
                  ...
                  <div>
                    <label className="block text-xs text-gray-700 dark:text-gray-300 font-medium">주제</label>
                    <div className="flex gap-2 items-center">
                      ...
                      <button
                        ...
                        className="mt-1 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-2 py-1 text-xs text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700"
                        title="토픽 새로고침"
                      >새로고침</button>
                    </div>
                    {topicsLoading && <div className="text-[11px] text-gray-500 dark:text-gray-400 mt-1">토픽 로딩중…</div>}
                    {isAdmin && (
                      <div className="mt-2 flex flex-wrap items-end gap-2 text-[11px] text-gray-700 dark:text-gray-300">
                        <label className="flex items-center gap-1">
                          <span>Org ID</span>
                          <input
                            type="number"
                            className="w-24 rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-1 py-0.5"
                            value={orgId}
                        ...
                        <button
                          type="button"
                          className="ml-auto rounded border border-gray-300 dark:border-gray-600 px-2 py-1 text-[11px] hover:bg-gray-50 dark:hover:bg-gray-700"
                          onClick={async () => {
                            try {
                  ...
                  <div>
                    <label className="block text-xs text-gray-700 dark:text-gray-300 font-medium">태그(콤마)</label>
                    <input
                      className="mt-1 w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 p-2 text-sm focus:border-blue-500 dark:focus:border-blue-400 focus:outline-none"
                  ...
                  <div>
                    <label className="block text-xs text-gray-700 dark:text-gray-300 font-medium">상태</label>
                    <select
                      className="mt-1 w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 p-2 text-sm focus:border-blue-500 dark:focus:border-blue-400 focus:outline-none"
                      value={form.status}
                ...
                <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  <div className="sm:col-span-2 lg:col-span-3">
                    <div className="rounded border border-amber-200 dark:border-amber-700 bg-amber-50 dark:bg-amber-900/20 px-3 py-2 text-[12px] text-amber-900 dark:text-amber-200 flex items-center gap-2" role="note" aria-live="polite" data-testid="irt-guide">
                      <div className="relative inline-block group">
                        ...
                        <div
                          id="irt-tooltip"
                          role="tooltip"
                          className="absolute left-0 mt-2 w-80 rounded-md border border-amber-200 dark:border-amber-700 bg-white dark:bg-gray-800 p-3 text-[12px] text-gray-800 dark:text-gray-200 shadow-lg invisible opacity-0 group-hover:visible group-hover:opacity-100 group-focus-within:visible group-focus-within:opacity-100 transition z-10"
                        >
                          ...
                          <div className="mt-2 text-[11px] text-gray-500 dark:text-gray-400">충분한 데이터로 자동 보정됩니다. 필요한 경우만 신중히 조정하세요.</div>
                        </div>
                      </div>
                      <span>
                        문항 모수(난이도 b, 변별도 a, 추측도 c)는 충분한 데이터로 자동 보정됩니다. 필요한 경우에만 신중히 조정하세요.
                        <a href="/docs/irt" target="_blank" rel="noopener noreferrer" className="ml-2 underline text-amber-900 dark:text-amber-300 hover:text-amber-800 dark:hover:text-amber-200" title="문항 모수 가이드 자세히 보기">자세히</a>
                      </span>
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs text-gray-700 dark:text-gray-300 font-medium">변별도 a (선택)</label>
                    <input
                      type="number"
                      step="0.01"
                      min="0"
                      max="5"
                      className="mt-1 w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 p-2 text-sm focus:border-blue-500 dark:focus:border-blue-400 focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed"
                      value={form.discrimination ?? ""}
                      onChange={(e) => setForm({ ...form, discrimination: e.target.value === '' ? undefined : Number(e.target.value) })}
                      data-testid="input-a"
                      disabled={!isAdmin || disabled}
                    />
                    <p className="mt-1 text-[11px] text-gray-500 dark:text-gray-400">권장 범위: 0.5 ~ 2.5</p>
                    {aWarn && <p className="mt-0.5 text-[11px] text-amber-700 dark:text-amber-400" data-testid="warn-a">권장 범위를 벗어났습니다.</p>}
                    {!isAdmin && <p className="mt-0.5 text-[11px] text-amber-700 dark:text-amber-400" data-testid="irt-readonly-a">관리자만 편집할 수 있습니다 (읽기 전용)</p>}
                  </div>
                  <div>
                    <label className="block text-xs text-gray-700 dark:text-gray-300 font-medium">난이도 b (선택)</label>
                    <input
                      type="number"
                      step="0.01"
                      min="-10"
                      ...
                      disabled={!isAdmin || disabled}
                    />
                    <p className="mt-1 text-[11px] text-gray-500 dark:text-gray-400">권장 범위: -3 ~ 3</p>
                    {bWarn && <p className="mt-0.5 text-[11px] text-amber-700 dark:text-amber-400" data-testid="warn-b">권장 범위를 벗어났습니다.</p>}
                    {!isAdmin && <p className="mt-0.5 text-[11px] text-amber-700 dark:text-amber-400" data-testid="irt-readonly-b">관리자만 편집할 수 있습니다 (읽기 전용)</p>}
                  </div>
                  <div>
                    <label className="block text-xs text-gray-700 dark:text-gray-300 font-medium">추측도 c (선택)</label>
                    <input
                      type="number"
                      step="0.01"
                      min="0"
                      ...
                      disabled={!isAdmin || disabled}
                    />
                    <p className="mt-1 text-[11px] text-gray-500 dark:text-gray-400">권장 범위: 0.00 ~ 0.35</p>
                    {cWarn && <p className="mt-0.5 text-[11px] text-amber-700 dark:text-amber-400" data-testid="warn-c">권장 범위를 벗어났습니다.</p>}
                    {!isAdmin && <p className="mt-0.5 text-[11px] text-amber-700 dark:text-amber-400" data-testid="irt-readonly-c">관리자만 편집할 수 있습니다 (읽기 전용)</p>}
                  </div>
                </div>
              </td>
            </tr>
            {/* 2) 제목 */}
            <tr>
              <th className="bg-gray-50 dark:bg-gray-800 text-left px-3 py-2 text-gray-600 dark:text-gray-300 align-top font-semibold">제목</th>
              <td className="px-3 py-2 bg-white dark:bg-gray-900">
                <RichTextEditor
                  value={form.title || ""}
                  ...
                  disabled={disabled}
                />
                <p className="mt-1 text-[11px] text-gray-500 dark:text-gray-400">제목은 간결하게 작성하세요. 필요 시 굵게/기울임, 수식 등을 사용할 수 있습니다.</p>
              </td>
            </tr>
            {/* 3) 본문 내용 */}
            <tr>
              <th className="bg-gray-50 dark:bg-gray-800 text-left px-3 py-2 text-gray-600 dark:text-gray-300 align-top font-semibold">본문 내용</th>
              <td className="px-3 py-2 bg-white dark:bg-gray-900">
                <RichTextEditor
                  value={form.stem}
                  ...
                  disabled={disabled}
                />
                <p className="mt-1 text-[11px] text-gray-500 dark:text-gray-400">본문은 HTML/MathML을 사용할 수 있습니다. 수식 버튼을 클릭하여 수학 수식을 삽입하세요.</p>
              </td>
            </tr>
            {/* 4) 해설 (선택) */}
            <tr>
              <th className="bg-gray-50 dark:bg-gray-800 text-left px-3 py-2 text-gray-600 dark:text-gray-300 align-top font-semibold">해설 (선택)</th>
              <td className="px-3 py-2 bg-white dark:bg-gray-900">
                <RichTextEditor
            ...
            {/* 5) 보기 / 정답 */}
            <tr>
              <th className="bg-gray-50 dark:bg-gray-800 text-left px-3 py-2 text-gray-600 dark:text-gray-300 align-top font-semibold">보기 / 정답</th>
              <td className="px-3 py-2 bg-white dark:bg-gray-900">
                <div className="space-y-3">
                  {form.options.map((opt, i) => (
                    <div key={i} className="flex items-center gap-3">
                      ...
                      />
                      <input
                        className="flex-1 rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 p-2 text-sm focus:border-blue-500 dark:focus:border-blue-400 focus:outline-none"
                        placeholder={`선지 ${i + 1}`}
                        value={opt}
                        onChange={(e) => {
                      ...
                      {canRemove && (
                        <button
                          ...
                          className="rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-2 py-1 text-xs text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700"
                ...
                <div className="mt-3">
                  <button
                    type="button"
                    onClick={addOption}
                    className="rounded-md border border-dashed border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-1.5 text-xs text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700"
                  >
                    + 보기 추가
                  </button>
                </div>
            ...
            {/* 6) 힌트 */}
            <tr>
              <th className="bg-gray-50 dark:bg-gray-800 text-left px-3 py-2 text-gray-600 dark:text-gray-300 align-top font-semibold">힌트 (선택)</th>
              <td className="px-3 py-2 bg-white dark:bg-gray-900">
                <RichTextEditor
                  value={form.hint || ""}
            ...
            {/* 7) 자료/리소스 */}
            <tr>
              <th className="bg-gray-50 dark:bg-gray-800 text-left px-3 py-2 text-gray-600 dark:text-gray-300 align-top font-semibold">자료/리소스 (선택)</th>
              <td className="px-3 py-2 bg-white dark:bg-gray-900">
                <RichTextEditor
                  value={form.resource || ""}
            ...
            {/* 8) 정답 서술 */}
            <tr>
              <th className="bg-gray-50 dark:bg-gray-800 text-left px-3 py-2 text-gray-600 dark:text-gray-300 align-top font-semibold">정답 서술 (선택)</th>
              <td className="px-3 py-2 bg-white dark:bg-gray-900">
                <RichTextEditor
                  ...
                  disabled={disabled}
                />
                <p className="mt-1 text-[11px] text-gray-500 dark:text-gray-400">주의: 위 입력은 정답 선택(라디오 버튼)과 별도입니다.</p>
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <div className="flex items-center justify-between pt-2">
        <div className="text-xs text-gray-500 dark:text-gray-400">
          필수: 본문, 보기 2개 이상, 정답 1개 선택
          {!isValid && errors.length > 0 && (
            <div className="mt-2 text-red-600 dark:text-red-400">
              <div className="font-semibold">저장 불가 사유:</div>
        ...
        <button
          ...
          className="rounded-md bg-blue-600 dark:bg-blue-500 px-3 py-1.5 text-white hover:bg-blue-700 dark:hover:bg-blue-600 disabled:cursor-not-allowed disabled:opacity-50 transition-colors"
          type="submit"
        >
          저장
        </button>
      ...
      {dirty && (
        <div className="fixed bottom-4 right-4 rounded bg-amber-100 dark:bg-amber-900/30 text-amber-900 dark:text-amber-200 border border-amber-200 dark:border-amber-700 text-xs px-3 py-2 shadow pointer-events-none">
          저장되지 않은 변경 사항이 있습니다.
        </div>
      )}
      {toast && (
        <div
          className={
            ...
            (toast.type === 'success' ? 'bg-emerald-600 dark:bg-emerald-500 text-white' : 'bg-rose-600 dark:bg-rose-500 text-white')
          }
          role="status"
