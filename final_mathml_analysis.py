#!/usr/bin/env python3
"""
Final MathML Analysis
Comprehensive analysis of the mpcstudy_db.sql file
"""

import re
import json

def analyze_mathml_final():
    """
    ...
    # Count total questions - look for INSERT INTO tbl_question
    total_questions = len(re.findall(r'INSERT INTO `tbl_question`', content))
    
    # Count MathML tags
    mathml_tags = re.findall(r'<math[^>]*>.*?</math>', content, re.DOTALL)
    total_mathml_tags = len(mathml_tags)
    
    # Count questions with MathML by looking at individual INSERT statements
    questions_with_mathml = 0
    
    # Split content into individual INSERT statements
    insert_statements = re.split(r'INSERT INTO `tbl_question`', content)[1:]  # Skip first empty part
    
    for statement in insert_statements:
        if '<math' in statement:
    ...
    grade_9 = len(re.findall(r"'G09'", content))
    grade_10 = len(re.findall(r"'G10'", content))
    grade_11 = len(re.findall(r"'G11'", content))
    grade_12 = len(re.findall(r"'G12'", content))
    
    print(f"\n📚 GRADE LEVEL BREAKDOWN:")
    print(f"   Grade 9: {grade_9:,}")
    print(f"   Grade 10: {grade_10:,}")
    print(f"   Grade 11: {grade_11:,}")
    print(f"   Grade 12: {grade_12:,}")
