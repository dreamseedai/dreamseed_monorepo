    ...
    def convert_mathml_to_mathlive(self, mathml_content: str) -> dict:
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                model="gpt-4o-mini",  # gpt-4.1-mini 대신 사용 가능한 모델
                messages=[
                    {
                        "role": "system", 
...
"""
    ...
    def _calculate_confidence(self, mathml: str, latex: str) -> float:
        ...
        complexity = sum(1 for indicator in complexity_indicators if indicator in mathml)
    ...
    def process_html_with_mathml(self, html_content: str) -> str:
        ...
        for mathml in mathml_tags:
            ...
            if result["status"] == "success":
                processed_content = processed_content.replace(mathml, result["converted_latex"])
            else:
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            "successful_conversions": sum(1 for result in self.cache.values() if result["status"] == "success"),
            "total_tokens_used": sum(result.get("api_usage", {}).get("total_tokens", 0) for result in self.cache.values())
        }
