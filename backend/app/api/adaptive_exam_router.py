...
from app.models.core_models_expanded import (
    Student, ExamSession, Attempt, Item, Class
)
from app.schemas.exam_schemas import (
    ...
    ExamSessionSummary, AttemptSummary, ItemResponse
)
...
@router.post("/exams/start", response_model=ExamStartResponse, status_code=status.HTTP_201_CREATED)
...
async def get_next_item(
    ...
    if not next_item:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="No suitable items available"
        )
...
async def get_exam_session(
    ...
    return ExamSessionSummary(
        ...
        standard_error=float(exam_session.standard_error) if exam_session.standard_error else None,
...
async def complete_exam_manually(
    session_id: int,
    db: Session = Depends(get_db)
):
    """
