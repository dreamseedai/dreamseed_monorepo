...
@router.post("/answer", response_model=AttemptResponse, status_code=status.HTTP_201_CREATED)
def submit_answer(
...
def get_exam_session(
    ...
    """
    query = select(ExamSession).where(ExamSession.id == session_id).options(
        selectinload(ExamSession.attempts)
    )
...
def get_student_exam_history(
    ...
    if current_user.role != "student":
        raise HTTPException(
            status_code=403,
            detail="Only students can view their exam history"
        )
...
def get_next_item(
    ...
    if exam_session.status != "in_progress":  # type: ignore
        raise HTTPException(
            status_code=400,
            detail="Exam is not in progress"
        )
