...
def enroll_student(enrollment: StudentClassroomCreate, db: Session = Depends(get_db)):
    ...
    if not clazz:
        raise HTTPException(
            status_code=404, detail=f"Class {enrollment.class_id} not found"
        )
...
def bulk_enroll_students(request: BulkEnrollmentRequest, db: Session = Depends(get_db)):
    ...
    if not clazz:
        raise HTTPException(
            status_code=404, detail=f"Class {request.class_id} not found"
        )
    ...
    for student_id in request.student_ids:
        try:
            enrollment = StudentClassroom(
                student_id=student_id, class_id=request.class_id
            )
            db.add(enrollment)
            db.commit()
            success_count += 1
...
def list_class_students(class_id: int, db: Session = Depends(get_db)):
    ...
    query = select(StudentClassroom.student_id).where(
...
@router.delete(
    "/enrollments/{student_id}/{class_id}", status_code=status.HTTP_204_NO_CONTENT
)
...
def create_exam_session(exam: ExamSessionCreate, db: Session = Depends(get_db)):
    ...
    if not student:
        raise HTTPException(
            status_code=404, detail=f"Student {exam.student_id} not found"
        )
...
def update_exam_session(
    session_id: int, exam_update: ExamSessionUpdate, db: Session = Depends(get_db)
):
...
def create_attempt(attempt: AttemptCreate, db: Session = Depends(get_db)):
    ...
    if not exam:
        raise HTTPException(
            status_code=404, detail=f"Exam session {attempt.exam_session_id} not found"
        )
...
@router.get(
    "/exam-sessions/{session_id}/attempts", response_model=List[AttemptResponse]
)
def list_session_attempts(session_id: int, db: Session = Depends(get_db)):
    """Get all attempts for an exam session"""
    query = (
        select(Attempt)
        .where(Attempt.exam_session_id == session_id)
...
def update_attempt(
    attempt_id: int, attempt_update: AttemptUpdate, db: Session = Depends(get_db)
):
...
def get_student_exam_stats(student_id: int, db: Session = Depends(get_db)):
    ...
    query = select(
        ...
        .label("completed_exams"),
        func.avg(ExamSession.score)
        .filter(ExamSession.status == "completed")
        .label("average_score"),
        func.avg(ExamSession.theta)
        .filter(ExamSession.status == "completed")
        .label("average_theta"),
...
def get_class_exam_stats(class_id: int, db: Session = Depends(get_db)):
    ...
    student_count = db.execute(
        select(func.count(StudentClassroom.student_id)).where(
            StudentClassroom.class_id == class_id
        )
    ).scalar_one()
