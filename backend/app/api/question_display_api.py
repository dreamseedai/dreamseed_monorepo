        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            if grade:
                where_conditions.append(
                    "(q.us_grade_level = %s OR q.ca_grade_level = %s)"
                )
            ...
            if not current_question:
                raise HTTPException(
                    status_code=404, detail="조건에 맞는 문제를 찾을 수 없습니다"
                )
            ...
            previous_question_id = (
                str(previous_question["id"]) if previous_question else None
            )
            ...
            total_val = (
                total_row["total"]
                if isinstance(total_row, dict) and "total" in total_row
                else 0
            )
            ...
            pos_val = (
                pos_row["position"]
                if isinstance(pos_row, dict) and "position" in pos_row
                else 1
            )
            ...
            return NavigationResponse(
                current_question=QuestionResponse(
                    ...
                    metadata={
                        "difficulty_score": (
                            float(current_question["difficulty_score"])
                            if current_question["difficulty_score"]
                            else 0.5
                        ),
