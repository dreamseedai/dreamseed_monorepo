...
class GPTClassificationSystem:
    ...
    def create_enhanced_classification_prompt(
        ...
        hints_text = " ".join(
            [hint.get("en", "") for hint in hints_en if isinstance(hint, dict)]
        )
        ...
        current_difficulty = (
            question_data.get("metadata", {}).get("difficulty", {}).get("level", "")
        )
...
"""
    ...
    def _get_curriculum_context(self) -> str:
        ...
        ontario_topics = self.curriculum_standards.get_grade_topics(
            "Ontario", "Mathematics", "Grade_12"
        )
        us_topics = self.curriculum_standards.get_grade_topics(
            "US", "Mathematics", "Grade_12"
        )
    ...
    async def classify_question_async(
        ...
        try:
            ...
            if cache_key in self.classification_cache:
                logger.info(
                    f"Using cached classification for question {question_data.get('id', 'unknown')}"
                )
            ...
            response = await asyncio.to_thread(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert educational content classifier. Always respond with valid JSON only.",
                    },
            ...
            logger.info(
                f"Successfully classified question {question_data.get('id', 'unknown')} in {processing_time:.2f}s"
            )
            return result

        except json.JSONDecodeError as e:
            logger.error(
                f"JSON decode error for question {question_data.get('id', 'unknown')}: {e}"
            )
            return self._create_error_result(str(e), time.time() - start_time)
        except Exception as e:
            logger.error(
                f"Error classifying question {question_data.get('id', 'unknown')}: {e}"
            )
            return self._create_error_result(str(e), time.time() - start_time)
    ...
    async def classify_questions_batch_async(
        self,
        ...
        for i in range(0, total_questions, batch_size):
            ...
            for j, res in enumerate(batch_results):
                if isinstance(res, Exception):
                    logger.error(f"Error in batch {batch_num}, question {j}: {res}")
                    err = self._create_error_result(str(res), 0)
                    err["original_question_id"] = batch[j].get("id")
                    results.append(err)
                else:
                    data = cast(Dict[str, Any], res)
                    data["original_question_id"] = batch[j].get("id")
                    results.append(data)
    ...
    def classify_questions_batch(
        ...
        """
        return asyncio.run(
            self.classify_questions_batch_async(questions_data, batch_size, delay)
        )
    ...
    def _create_error_result(
        self, error_message: str, processing_time: float
    ) -> Dict[str, Any]:
        """
    ...
    def validate_classification_quality(
        ...
        for result in results:
            if "error" not in result:
                if result.get("ontario_classification", {}).get("confidence"):
                    ontario_confidences.append(
                        result["ontario_classification"]["confidence"]
                    )
        ...
        for result in results:
            if "error" not in result:
                ontario_diff = result.get("ontario_classification", {}).get(
                    "difficulty_level"
                )
                us_diff = result.get("us_classification", {}).get("difficulty_level")

                if ontario_diff:
                    ontario_difficulties[ontario_diff] = (
                        ontario_difficulties.get(ontario_diff, 0) + 1
                    )
                if us_diff:
        ...
        for result in results:
            if "error" not in result and "quality_assessment" in result:
                ...
                for metric in quality_scores.keys():
                    if metric in qa:
                        value = qa[metric]
                        quality_scores[metric][value] = (
                            quality_scores[metric].get(value, 0) + 1
                        )
        ...
        validation_report = {
            "summary": {
                ...
                "success_rate": (
                    successful_classifications / total_questions
                    if total_questions > 0
                    else 0
                ),
            },
            "confidence_analysis": {
                "ontario_avg_confidence": (
                    sum(ontario_confidences) / len(ontario_confidences)
                    if ontario_confidences
                    else 0
                ),
                "us_avg_confidence": (
                    sum(us_confidences) / len(us_confidences) if us_confidences else 0
                ),
                "high_confidence_ontario": len(
                    [c for c in ontario_confidences if c >= 0.8]
                ),
            ...
            "recommendations": self._generate_quality_recommendations(
                results, ontario_confidences, us_confidences
            ),
        }
    ...
    def _generate_quality_recommendations(
        ...
        if ontario_confidences:
            ...
            if avg_ontario_confidence < 0.8:
                recommendations.append(
                    "Ontario classification confidence is below 0.8. Consider manual review of low-confidence classifications."
                )
        ...
        if us_confidences:
            ...
            if avg_us_confidence < 0.8:
                recommendations.append(
                    "US classification confidence is below 0.8. Consider manual review of low-confidence classifications."
                )
        ...
        if error_count > len(results) * 0.1:  # More than 10% errors
            recommendations.append(
                f"High error rate ({error_count} errors). Check API connectivity and prompt quality."
            )
        ...
        if low_quality_count > len(results) * 0.2:  # More than 20% low quality
            recommendations.append(
                f"Many questions rated as low quality ({low_quality_count}). Consider content review and improvement."
            )
...
def main():
    ...
    # Classify questions
    results = classifier.classify_questions_batch(
    ...
    with open(
        f"gpt_classification_results_{timestamp}.json", "w", encoding="utf-8"
    ) as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    ...
    print("=" * 60)
    print(
        f"Total Questions Processed: {validation_report['summary']['total_questions']}"
    )
    print(
        f"Successful Classifications: {validation_report['summary']['successful_classifications']}"
    )
    print(f"Success Rate: {validation_report['summary']['success_rate']:.2%}")

    print(f"\nConfidence Analysis:")
    print(
        f"  Ontario Average Confidence: {validation_report['confidence_analysis']['ontario_avg_confidence']:.2f}"
    )
    print(
        f"  US Average Confidence: {validation_report['confidence_analysis']['us_avg_confidence']:.2f}"
    )
    print(
        f"  High Confidence Ontario: {validation_report['confidence_analysis']['high_confidence_ontario']}"
    )
    print(
        f"  High Confidence US: {validation_report['confidence_analysis']['high_confidence_us']}"
    )
