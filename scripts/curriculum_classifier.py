...
"""
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert educational content classifier. Always respond with valid JSON.",
                    },
        ...
        for i in range(0, total_questions, batch_size):
            ...
            for j, question in enumerate(batch):
                try:
                    ...
                    if (j + 1) % 5 == 0:
                        logger.info(
                            f"  Processed {j + 1}/{len(batch)} questions in batch"
                        )

                except Exception as e:
                    logger.error(
                        f"Error processing question {question.get('id', 'unknown')}: {e}"
                    )
                    batch_results.append(
                        {"original_question_id": question.get("id"), "error": str(e)}
                    )
    ...
    def generate_classification_report(
        ...
        for result in results:
            ...
            if result.get("us_classification"):
                ...
                us_grades[us_class.get("grade", "Unknown")] = (
                    us_grades.get(us_class.get("grade", "Unknown"), 0) + 1
                )
                us_subjects[us_class.get("subject", "Unknown")] = (
                    us_subjects.get(us_class.get("subject", "Unknown"), 0) + 1
                )
                us_courses[us_class.get("course", "Unknown")] = (
                    us_courses.get(us_class.get("course", "Unknown"), 0) + 1
                )
                us_topics[us_class.get("topic", "Unknown")] = (
                    us_topics.get(us_class.get("topic", "Unknown"), 0) + 1
                )
            ...
            if result.get("canada_classification"):
                ...
                canada_grades[canada_class.get("grade", "Unknown")] = (
                    canada_grades.get(canada_class.get("grade", "Unknown"), 0) + 1
                )
                canada_subjects[canada_class.get("subject", "Unknown")] = (
                    canada_subjects.get(canada_class.get("subject", "Unknown"), 0) + 1
                )
                canada_courses[canada_class.get("course", "Unknown")] = (
                    canada_courses.get(canada_class.get("course", "Unknown"), 0) + 1
                )
                canada_topics[canada_class.get("topic", "Unknown")] = (
                    canada_topics.get(canada_class.get("topic", "Unknown"), 0) + 1
                )
            ...
            if result.get("difficulty_assessment"):
                ...
                us_difficulties[diff_assess.get("us_difficulty", "Unknown")] = (
                    us_difficulties.get(diff_assess.get("us_difficulty", "Unknown"), 0)
                    + 1
                )
                canada_difficulties[diff_assess.get("canada_difficulty", "Unknown")] = (
                    canada_difficulties.get(
                        diff_assess.get("canada_difficulty", "Unknown"), 0
                    )
                    + 1
                )
        ...
        report = {
            ...
            "canada_curriculum_distribution": {
                ...
                "average_confidence": (
                    sum(canada_confidences) / len(canada_confidences)
            ...
            "curriculum_comparison": {
                ...
                "subject_alignment": self._calculate_alignment(
                    us_subjects, canada_subjects
                ),
                "difficulty_alignment": self._calculate_alignment(
                    us_difficulties, canada_difficulties
                ),
            },
    ...
    def _generate_recommendations(self, results: List[Dict[str, Any]]) -> List[str]:
        ...
        us_confidences = [
            r.get("us_classification", {}).get("confidence", 0)
            for r in results
            if "us_classification" in r
        ]
        canada_confidences = [
            r.get("canada_classification", {}).get("confidence", 0)
            for r in results
            if "canada_classification" in r
        ]
        ...
        if us_confidences:
            ...
            if avg_us_confidence < 0.8:
                recommendations.append(
                    "US classification confidence is low. Consider manual review of low-confidence classifications."
                )
        ...
        if canada_confidences:
            ...
            if avg_canada_confidence < 0.8:
                recommendations.append(
                    "Canada classification confidence is low. Consider manual review of low-confidence classifications."
                )
        ...
        if len(us_subjects) < 4:
            recommendations.append(
                "Limited subject coverage in US classification. Consider expanding question diversity."
            )

        if len(canada_subjects) < 4:
            recommendations.append(
                "Limited subject coverage in Canada classification. Consider expanding question diversity."
            )
...
def main():
    ...
    with open(
        f"curriculum_classification_results_{timestamp}.json", "w", encoding="utf-8"
    ) as f:
        json.dump(results, f, indent=2, ensure_ascii=False)

    with open(
        f"curriculum_classification_report_{timestamp}.json", "w", encoding="utf-8"
    ) as f:
    ...
    print("=" * 60)
    print(
        f"Total Questions Processed: {report['classification_summary']['total_questions']}"
    )
    print(
        f"Successful Classifications: {report['classification_summary']['successful_classifications']}"
    )
    ...
    print(
        f"  Average Confidence: {report['us_curriculum_distribution']['average_confidence']:.2f}"
    )
    ...
    print(
        f"  Average Confidence: {report['canada_curriculum_distribution']['average_confidence']:.2f}"
    )
