...
BASE="apps/seedtest-api"
mkdir -p "$BASE"/{app,components,lib,schemas,routers,services,security,middleware}
mkdir -p scripts

# ---------------- requirements ----------------
cat > "$BASE/requirements.txt" <<'PY'
fastapi==0.114.0
...
cat > "$BASE/services/adaptive_engine.py" <<'PY'
import time
...
cat > "$BASE/middleware/correlation.py" <<'PY'
...
# ---------------- helper script to run locally ----------------
cat > "scripts/run_seedtest_api.sh" <<'SH'
#!/usr/bin/env bash
set -euo pipefail
...
SH
chmod +x scripts/run_seedtest_api.sh

echo "✅ SeedTest FastAPI scaffold created at $BASE"
echo "Run: bash scripts/run_seedtest_api.sh"
