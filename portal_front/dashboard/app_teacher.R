...
source("data_access_minimal.R", local = TRUE)
...
filter_by_access <- function(ds, claims) {
  # Restrict by org_id only if column exists
  if (!is.null(claims$org_id) && nzchar(claims$org_id) && "org_id" %in% names(ds)) {
...
server <- function(input, output, session) {
  ...
  USE_MIN <- use_min_schema() && (min_backend() == "db" || min_arrow_exists())
  ...
  # Data sources (demo vs minimal schema)
  classes_ds <- reactive({
    if (USE_MIN) {
      filter_by_access(tbl_classes_index(), claims)
    } else {
      filter_by_access(open_ds(base, "classes"), claims)
    }
  })
  ...
  theta_ds <- reactive({
    if (USE_MIN) {
      # Join student to snap if class_id not present
      snap <- filter_by_access(tbl_irt_snapshot(), claims)
      snap_cols <- try(names(snap), silent = TRUE)
      if (!inherits(snap_cols, "try-error") && !("class_id" %in% snap_cols)) {
        stu <- filter_by_access(tbl_student(), claims) %>% dplyr::select(student_id = id, class_id)
        snap <- snap %>% dplyr::inner_join(stu, by = "student_id")
      }
      snap %>% dplyr::filter(class_id == !!input$class_id)
    } else {
      filter_by_access(open_ds(base, "student_theta"), claims) %>% dplyr::filter(class_id == !!input$class_id)
    }
  })
  students_ds <- reactive({
    if (USE_MIN) {
      filter_by_access(tbl_student(), claims) %>%
        dplyr::filter(class_id == !!input$class_id) %>%
        dplyr::transmute(student_id = id, student_name = name, class_id = class_id, grade = grade, country = "USA", language = "en-US")
    } else {
      filter_by_access(open_ds(base, "students"), claims) %>% dplyr::filter(class_id == !!input$class_id)
    }
  })
  attend_ds <- reactive({
    if (USE_MIN) {
      filter_by_access(tbl_attendance_joined(), claims) %>% dplyr::filter(class_id == !!input$class_id)
    } else {
      filter_by_access(open_ds(base, "attendance"), claims) %>% dplyr::filter(class_id == !!input$class_id)
    }
  })
  skill_ds <- reactive({
    if (USE_MIN) {
      # already computed top3 per student; filter after join
      tbl_skill_weakness() %>% dplyr::filter(TRUE)
    } else {
      filter_by_access(open_ds(base, "skill_weakness"), claims) %>% dplyr::filter(class_id == !!input$class_id)
    }
  })
  resp_ds <- reactive({
    if (USE_MIN) {
      filter_by_access(tbl_response_stats(), claims) %>% dplyr::filter(student_id %in% (students_ds() %>% dplyr::select(student_id) %>% collect() %>% .$student_id))
    } else {
      filter_by_access(open_ds(base, "response_stats"), claims) %>% dplyr::filter(class_id == !!input$class_id)
    }
  })
  item_resp_ds <- reactive({
    # Minimal schema doesn't include item-level patterns; keep demo dataset
    filter_by_access(open_ds(base, "item_response_patterns"), claims) %>% dplyr::filter(class_id == !!input$class_id)
  })
  ...
  attn_var_cutoff <- reactive({
    ...
    raw_att <- tryCatch({
      if (USE_MIN) {
        collect_subgroup_data(tbl_attendance_joined(), current_country, current_subject, current_grade, min_rows = 50)
      } else {
        collect_subgroup_data(open_ds(base, "attendance"), current_country, current_subject, current_grade, min_rows = 50)
      }
    }, error = function(e) {
    ...
    if (nrow(raw_att) < 10 || !all(c("student_id","date","status") %in% names(raw_att))) {
      # 데이터 부족 시 현재 클래스만 사용
    ...
    # 서브그룹의 학생별 요일 분산 점수 계산
    var_by_student <- raw_att %>%
      dplyr::mutate(is_abs = status == "absent", is_tardy = status %in% c("tardy","late"),
                    weekday = lubridate::wday(date, label = TRUE, abbr = TRUE, week_start = 1)) %>%
      dplyr::group_by(student_id, weekday) %>%
      dplyr::summarise(abs_rate_dow = mean(is_abs), tardy_rate_dow = mean(is_tardy), .groups='drop') %>%
      dplyr::group_by(student_id) %>%
      dplyr::summarise(var_score = max(var(abs_rate_dow, na.rm = TRUE), var(tardy_rate_dow, na.rm = TRUE), na.rm = TRUE), .groups='drop')

    pct <- get_risk_threshold(CONFIG, current_country, current_subject, "attendance_var_top_pct")
    cutoff <- as.numeric(stats::quantile(var_by_student$var_score, probs = pct, na.rm = TRUE))
    message("[attn_var_cutoff] ✓ Subgroup (", current_country, ".", current_subject, ".", current_grade,
            "): N=", nrow(var_by_student), ", cutoff=", round(cutoff, 4), " (", pct*100, "th percentile)")
    cutoff
  })
  ...
  guess_q_cutoff <- reactive({
    ...
    rsp_sub <- tryCatch({
      if (USE_MIN) {
        # derive from irt_snapshot latest
        irt <- tbl_irt_snapshot()
        latest_w <- irt %>% group_by(student_id) %>% summarise(week_start = max(week_start, na.rm = TRUE), .groups='drop')
        irt %>% inner_join(latest_w, by = c("student_id","week_start")) %>%
          transmute(student_id, guess_like_rate = coalesce(c_hat, 0)) %>% collect()
      } else {
        collect_subgroup_data(open_ds(base, "response_stats"), current_country, current_subject, current_grade, min_rows = 10)
      }
    }, error = function(e) {
    ...
    if (nrow(rsp_sub) < 10 || !("guess_like_rate" %in% names(rsp_sub))) {
    ...
    cutoff <- as.numeric(stats::quantile(rsp_sub$guess_like_rate, probs = pct, na.rm = TRUE))
    message("[guess_q_cutoff] ✓ Subgroup (", current_country, ".", current_subject, ".", current_grade,
            "): N=", nrow(rsp_sub), ", cutoff=", round(cutoff, 4), " (", pct*100, "th percentile)")
    cutoff
  })

  # Latest theta per student and deltas + 3-week non-positive streak flag
  latest_theta_tbl <- reactive({
    req(input$class_id)
    dr <- date_filter()
    if (USE_MIN) {
      # Filter by week range around date range
      df <- theta_ds() %>%
        dplyr::filter(week_start >= !!(dr[1] - 21), week_start <= !!dr[2]) %>%
        collect()
      if (nrow(df) == 0) return(tibble(student_id = character(), theta = numeric(), delta_7d = numeric(), three_nonpos = logical()))
      # latest snapshot per student
      latest_w <- df %>% group_by(student_id) %>% filter(week_start == max(week_start, na.rm = TRUE)) %>% summarise(theta = dplyr::first(theta), delta_7d = dplyr::first(delta_theta), .groups='drop')
      # compute 3-week non-positive streak using last 28 days of weekly data
      maxw <- max(df$week_start, na.rm = TRUE)
      weekly <- df %>%
        filter(week_start > (maxw - 28)) %>%
        arrange(student_id, week_start) %>%
        group_by(student_id) %>%
        mutate(delta_w = theta - dplyr::lag(theta)) %>%
        summarise(three_nonpos = {
          d <- delta_w[!is.na(delta_w)]
          if (length(d) >= 3) all(tail(d, 3) <= 0) else FALSE
        }, .groups='drop')
      out <- latest_w %>% left_join(weekly, by = "student_id") %>% mutate(three_nonpos = ifelse(is.na(three_nonpos), FALSE, three_nonpos))
      out
    } else {
      df_range <- theta_ds() %>% dplyr::filter(date >= !!dr[1], date <= !!dr[2]) %>% collect()

      latest <- df_range %>% group_by(student_id) %>% filter(date == max(date)) %>% summarise(theta = dplyr::first(theta), .groups='drop')
      week_ago_date <- max(df_range$date) - 7
      week_ago <- df_range %>% group_by(student_id) %>% filter(date <= !!week_ago_date) %>% filter(date == max(date)) %>% summarise(theta_7d = dplyr::first(theta), .groups='drop')
  ...
  attn_metrics_tbl <- reactive({
    ...
    overall <- adf %>% mutate(
      ...
      is_tardy = status %in% c("tardy","late")
  ...
  output$vb_theta_growth <- renderValueBox({
    dr <- date_filter()
    if (USE_MIN) {
      df <- theta_ds() %>% dplyr::filter(week_start >= !!(dr[1]-28), week_start <= !!dr[2]) %>% collect()
      if (nrow(df) == 0) return(valueBox("+0.00", "주간 성장률 Δθ", icon = icon("chart-line"), color = "green"))
      maxw <- max(df$week_start, na.rm = TRUE)
      end_week <- df %>% dplyr::filter(week_start == maxw)
      prev_week <- df %>% dplyr::filter(week_start == (maxw - 7))
      ...
      df <- theta_ds() %>% dplyr::filter(date >= !!(dr[1]-14), date <= !!dr[2]) %>% collect()
      end_week <- df %>% dplyr::filter(date > max(date)-7)
      prev_week <- df %>% dplyr::filter(date <= max(date)-7, date > max(date)-14)
      g <- mean(end_week$theta, na.rm = TRUE) - mean(prev_week$theta, na.rm = TRUE)
      valueBox(sprintf("%+.2f", g), "주간 성장률 Δθ", icon = icon("chart-line"), color = if (is.na(g) || g >= 0) "green" else "yellow")
    }
  })
  ...
  # Individual student assignment from table action button (GLOBAL EDITION)
  observeEvent(input$assign_single_student, {
    ...
    # 클래스 메타 조회 (subject, grade, subject_level 등)
    class_meta <- classes_ds() %>% 
      filter(class_id == !!student_meta$class_id[1]) %>% 
      collect()
    
    # 국가/과목/학년 정보 추출
    country <- student_meta$country[1] %||% class_meta$country[1] %||% "USA"
    subject <- class_meta$subject[1] %||% "math"
    grade <- student_meta$grade[1] %||% class_meta$grade[1] %||% "G9"
    level <- class_meta$subject_level[1]  # "honors", "AP", "algebra2", NULL
    language <- student_meta$language[1] %||% "en-US"
    
    # 템플릿 검색 (계층적: country.subject.grade.level.bucket)
    template <- get_template(CONFIG, country, subject, grade, level, theta_bucket)
    template_id <- template$template_id %||% "DEFAULT_CORE"
    
    # 학생 이름 (다국어 지원)
    student_name <- student_meta$student_name[1] %||% student_id
    ...
    # 다국어 메시지
    if (ok) {
      msg <- get_i18n_message(CONFIG, language, "assign_success", list(count = 1))
      msg_full <- sprintf("%s - %s (%s)", msg, student_name, template_id)
      showNotification(msg_full, type = "message", duration = 5)
    } else {
      msg <- get_i18n_message(CONFIG, language, "assign_fail")
