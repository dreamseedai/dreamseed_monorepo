...
echo "🔐 DreamSeed Alert Threader - 모든 언어 EnvironmentFile 통합 설치"

# 1. 언어 선택
echo "📋 설치할 언어를 선택하세요:"
echo "  1) Python (FastAPI) - EnvironmentFile"
...
for lang in "${LANGUAGES[@]}"; do
    case $lang in
        python)
            echo "  - Python EnvironmentFile 버전 복사 중..."
            sudo cp -r python-advanced-envfile/* /opt/alert-threader/python-advanced-envfile/
            sudo chown -R www-data:www-data /opt/alert-threader/python-advanced-envfile
            ;;
        nodejs)
...
for lang in "${LANGUAGES[@]}"; do
    case $lang in
        python)
            SERVICE_FILE="ops-services-alert-threader-python-envfile.service"
...
echo "🎉 DreamSeed Alert Threader 모든 언어 EnvironmentFile 통합 설치 완료!"
echo ""
echo "📋 설정 요약:"
...
echo "  7. 서비스 관리:"
echo "     # 모든 서비스 상태 확인"
echo "     for lang in python nodejs go; do"
echo "       echo \"=== \$lang ===\""
echo "       sudo systemctl status alert-threader-\$lang --no-pager"
echo "     done"
echo ""
echo "     # 서비스 전환 (예: Python → Node.js)"
echo "     sudo systemctl stop alert-threader-python"
echo "     sudo systemctl start alert-threader-node"
