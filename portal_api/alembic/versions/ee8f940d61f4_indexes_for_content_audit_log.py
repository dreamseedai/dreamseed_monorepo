...
def upgrade() -> None:
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_cal_user_id_id ON content_audit_log (user_id, id DESC)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_cal_content_id_id ON content_audit_log (content_id, id DESC)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_cal_action_id ON content_audit_log (action, id DESC)"
    )
