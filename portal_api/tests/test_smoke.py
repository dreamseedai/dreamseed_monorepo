...
os.environ.setdefault(
    "DATABASE_URL", "postgresql+psycopg://postgres:postgres@127.0.0.1:5432/dreamseed"
)
...
async def test_ok_and_auth_flow():
    ...
    async with AsyncClient(
        ...
        r = await ac.post(
            ...
            json={"title": "T1", "doc": doc},
        )
        assert r.status_code == 200
        cid = r.json()["id"]

        r = await ac.get("/content?limit=1", headers={"Authorization": f"Bearer {at}"})
        assert r.status_code == 200

        r = await ac.put(
            f"/content/{cid}",
            headers={"Authorization": f"Bearer {at}"},
            json={"title": "T1b", "doc": doc},
        )
        assert r.status_code == 200
        r = await ac.delete(
            f"/content/{cid}", headers={"Authorization": f"Bearer {at}"}
        )
        assert r.status_code == 200 and r.json().get("ok") is True
