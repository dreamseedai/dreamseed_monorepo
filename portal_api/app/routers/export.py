...
async def export_html(
    ...
    allowed = await check_rate_limit(
        f"export:{ip}", get_settings().export_rate_limit_per_min, 60
    )
    if not allowed:
...
async def export_pdf(
    ...
    headers = {
        "Content-Disposition": f'attachment; filename="content_{content_id}.pdf"'
    }
...
async def export_zip(
    ...
    buf.seek(0)
    headers = {
        "Content-Disposition": f'attachment; filename="export_{fmt}.zip"',
        "X-Export-Job": job_id,
    }
...
def _progress_set(job_id: str, val: int) -> None:
    ...
    if s.redis_url:
        ...
        async def _set():
            await r.setex(
                f"export:prog:{job_id}",
                int(os.getenv("EXPORT_PROGRESS_TTL", "600")),
                str(val),
            )
