...
def test_settings_read_and_write_with_admin_token(client, monkeypatch):
    ...
    new_policy = {"prefer_balanced": False, "deterministic": True, "max_per_topic": 2, "top_k_random": 3, "info_band_fraction": 0.07}
    r = client.patch("/api/settings/selection", json=new_policy)
    assert r.status_code == 401

    # With token, write succeeds
    r = client.patch("/api/settings/selection", headers={"X-Admin-Token": "secret"}, json=new_policy)
    assert r.status_code == 200
    got = r.json()
...
def test_settings_namespaces_with_stubbed_redis(client, monkeypatch):
    ...
    payload = json.dumps({"policy": {"prefer_balanced": True, "deterministic": False, "max_per_topic": None, "top_k_random": None, "info_band_fraction": 0.05}, "last_updated": "2025-01-01T00:00:00Z"})
    ...
    monkeypatch.setattr(cfg, "redis", types.SimpleNamespace(Redis=types.SimpleNamespace(from_url=lambda *a, **k: fake)), raising=False)
...
def test_reports_theta_se_png(client):
    r = client.get("/api/reports/theta-se.png", params=[("theta", 0.0), ("theta", 0.5), ("se", 0.6), ("se", 0.5), ("title", "t")])
    assert r.status_code == 200
...
def test_ability_estimator_se_accumulation():
    ...
    se2 = estimate_standard_error(theta=0.0, answered_items=[{"info": 0.5}, {"info": 2.0}])
    assert se2 <= se1
