...
def delete_selection(
    namespace: str | None = None, x_admin_token: str | None = Header(default=None)
) -> SelectionPolicy:
    _require_admin(x_admin_token)
...
def get_effective_settings(namespace: str | None = None) -> dict:
    ...
    p, source, resolved_ns, last = get_selection_policy_info(
        namespace=namespace, resolve_hierarchy=True
    )
    return {
...
def patch_estimator_settings(
    payload: dict, x_admin_token: str | None = Header(default=None)
):
    _require_admin(x_admin_token)
    s = get_settings()
...
def patch_irt_updater_settings(
    payload: dict, x_admin_token: str | None = Header(default=None)
):
    _require_admin(x_admin_token)
    s = get_settings()
    ...
    if "target_correct_rate" in payload and isinstance(
        payload["target_correct_rate"], (int, float)
    ):
        setattr(s, "irt_target_correct_rate", float(payload["target_correct_rate"]))
    if "learning_rate" in payload and isinstance(
        payload["learning_rate"], (int, float)
    ):
...
def list_namespaces(include_global: bool = True) -> dict:
    ...
    for ns in names:
        p, source, resolved_ns, last = get_selection_policy_info(
            namespace=(ns or None), resolve_hierarchy=True
        )
        policies[ns] = {
            "policy": p.model_dump(),
            "source": source,
