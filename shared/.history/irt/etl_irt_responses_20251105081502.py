...
class IRTDatabase:
    ...
    async def extract_responses(
        self, start_at: datetime, end_at: datetime, population_tags: List[str]
    ) -> List[dict]:
        """
    ...
    async def get_item_response_summary(
        self, responses: List[dict], min_responses: int, min_variance: float
        ...
        for item_id, data in item_data.items():
            ...
            if n < min_responses:
                logger.debug(
                    f"Item {item_id}: insufficient responses ({n} < {min_responses})"
                )
                continue
            ...
            if variance < min_variance:
                logger.debug(
                    f"Item {item_id}: low variance ({variance:.4f} < {min_variance})"
                )
                continue
    ...
    async def export_calibration_data(
        ...
        for resp in responses:
            ...
            if (
                key not in user_item_map
                or resp["answered_at"] > user_item_map[key]["answered_at"]
            ):
                user_item_map[key] = resp

        unique_responses = list(user_item_map.values())
        logger.info(
            f"Deduplicated to {len(unique_responses)} unique user-item responses"
        )

        # Write CSV
        with open(output_path, "w", newline="") as f:
            writer = csv.DictWriter(
                f, fieldnames=["user_id_hash", "item_id", "is_correct"]
            )
            writer.writeheader()
...
async def run_etl(
    ...
    try:
        ...
        responses = await db.extract_responses(
            window_spec.start_at, window_spec.end_at, window_spec.population_tags
        )
        ...
        summaries = await db.get_item_response_summary(
            responses, min_responses, min_variance
        )

        logger.info(
            f"Window {window_id} summary: {len(summaries)} items ready for calibration"
        )
...
@click.option(
    "--window-label", required=True, help='Window label (e.g., "2025-10 monthly")'
)
...
@click.option(
    "--population-tags",
    multiple=True,
    help='Population filters (e.g., "cohort:2025-Q4", "lang:ko")',
)
...
def main(
    ...
    # Run ETL
    asyncio.run(run_etl(database_url, window_spec, min_responses, min_variance, output))
