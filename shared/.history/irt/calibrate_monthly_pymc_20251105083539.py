...
class IRTDatabase:
    ...
    def create_window(self, label: Optional[str] = None) -> Dict:
        ...
        with self.engine.begin() as conn:
            ...
            result = conn.execute(
                text(
                    "SELECT id, start_at, end_at, label FROM shared_irt.windows WHERE label = :label"
                ),
                {"label": label},
            )
            ...
            if existing:
                logger.info(
                    f"Using existing window: {existing['label']} (id={existing['id']})"
                )
    ...
    def store_calibration(self, window_id: int, results: pd.DataFrame, loglik: float):
    ...
    def store_drift_alerts(self, window_id: int, alerts: List[Dict]):
        """Store drift alerts"""
        if not alerts:
    ...
    def update_current_params(self, window_id: int, stable_items: List[int]):
...
def fit_pymc_2pl(
    ...
    with pm.Model() as model:
        ...
        if anchor_priors is not None and not anchor_priors.empty:
            ...
            logger.info(
                f"Using informative priors for {len(anchor_priors)} anchor items"
            )
    ...
    if r_hat_max > 1.01:
        logger.warning(
            f"Warning: Max R-hat = {r_hat_max:.4f} > 1.01 (poor convergence)"
        )
    if ess_bulk_min < 400:
        logger.warning(
            f"Warning: Min ESS = {ess_bulk_min:.0f} < 400 (low effective sample size)"
        )
...
def run_calibration(
    ...
    logger.info(
        f"Final dataset: {len(df)} responses, {len(user_index)} users, {len(item_index)} items"
    )
    ...
    if not baseline.empty:
        merged = results.merge(
            baseline, on="item_id", how="inner", suffixes=("", "_prev")
        )
...
@click.option("--min-responses", default=200, help="Minimum responses per item")
@click.option("--samples", default=1000, help="MCMC samples (post-warmup)")
@click.option("--tune", default=1000, help="MCMC tuning/warmup samples")
@click.option("--chains", default=4, help="Number of MCMC chains")
@click.option("--target-accept", default=0.9, help="NUTS target acceptance rate")
@click.option(
    "--drift-threshold-b", default=0.25, help="Difficulty drift threshold |Δb|"
)
@click.option(
    "--drift-threshold-a", default=0.2, help="Discrimination drift threshold |Δa|"
)
def main(
    database_url: str,
