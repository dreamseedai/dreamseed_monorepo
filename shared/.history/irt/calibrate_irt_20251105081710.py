...
class IRTCalibrator:
    ...
    async def connect(self):
        ...
        self.pool = await asyncpg.create_pool(
            self.database_url, min_size=2, max_size=10, command_timeout=120
        )
    ...
    async def export_responses_for_window(self, window_id: int, output_csv: str) -> int:
    ...
    async def get_previous_params(self, window_id: int, output_json: str) -> bool:
        ...
        async with self.pool.acquire() as conn:
            ...
            if not params:
                logger.info(
                    f"No previous calibration found for window {prev_window_id}"
                )
                return False
    ...
    async def store_calibration_results(self, window_id: int, results_json: str):
    ...
    async def calibrate_window(self, window_id: int, model: str = "2PL"):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            n_responses = await self.export_responses_for_window(
                window_id, str(input_csv)
            )
