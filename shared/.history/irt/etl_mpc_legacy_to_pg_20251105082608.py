...
def options_to_tiptap(options: List[str]) -> Dict:
    ...
    for i, opt_html in enumerate(options, 1):
        ...
        content.append(
            {
                "type": "paragraph",
                "content": [{"type": "text", "text": f"{i}. {opt_text}"}],
            }
        )
...
class MPCLegacyETL:
    ...
    def __init__(
        ...
        self.stats = {"total": 0, "inserted": 0, "updated": 0, "errors": 0}
    ...
    def connect_mysql(self) -> Tuple[pymysql.Connection, pymysql.cursors.DictCursor]:
        ...
        cursor = conn.cursor()
        logger.info(
            f"Connected to MySQL: {self.mysql_config['host']}/{self.mysql_config['db']}"
        )
        return conn, cursor
    ...
    def connect_pg(
        ...
        cursor = conn.cursor()
        logger.info(
            f"Connected to PostgreSQL: {self.pg_config['host']}/{self.pg_config['dbname']}"
        )
        return conn, cursor
    ...
    def transform_item(self, row: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            # Build answer key
            answer_key = {"type": "mcq", "correct": row.get("answer", 1)}
...
@click.command()
@click.option(
    "--mysql-host", envvar="MPC_MYSQL_HOST", default="127.0.0.1", help="MySQL host"
)
@click.option(
    "--mysql-user", envvar="MPC_MYSQL_USER", default="root", help="MySQL user"
)
@click.option(
    "--mysql-password",
    envvar="MPC_MYSQL_PASSWORD",
    required=True,
    help="MySQL password",
)
@click.option(
    "--mysql-db", envvar="MPC_MYSQL_DB", default="mpc_legacy", help="MySQL database"
)
@click.option(
    "--pg-host", envvar="POSTGRES_HOST", default="127.0.0.1", help="PostgreSQL host"
)
@click.option(
    "--pg-user", envvar="POSTGRES_USER", default="postgres", help="PostgreSQL user"
)
@click.option(
    "--pg-password",
