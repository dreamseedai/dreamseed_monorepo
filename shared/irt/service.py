...
def item_info_curve(
    a: float, b: float, c: Optional[float], thetas: np.ndarray, model: IRTModel = "2PL"
) -> np.ndarray:
    """
...
def test_info_curve(items: List[dict], thetas: np.ndarray) -> np.ndarray:
    """
...
def optimal_theta_range(
    test_info: np.ndarray, thetas: np.ndarray, info_threshold: float = 10.0
) -> Tuple[float, float]:
    """
...
def standard_error_of_measurement(test_info: np.ndarray) -> np.ndarray:
    """
...
def select_next_item_mfi(remaining_items: List[dict], theta_current: float) -> int:
    """
...
def estimate_theta_eap(
    ...
    # Quadrature points
    thetas = np.linspace(
        prior_mean - 4 * prior_sd, prior_mean + 4 * prior_sd, quad_points
    )
...
def update_exposure_counts(conn: Connection, item_ids: List[int], increment: int = 1):
    """
...
def get_exposure_balanced_items(
    conn: Connection, candidate_items: List[int], max_exposure_rate: float = 0.3
) -> List[int]:
    """
