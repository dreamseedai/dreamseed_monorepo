...
class BayesianGrowthPredictor:
    ...
    def calculate_target_probability(
        ...
        if trials_to_success:
            expected = sum(t for t in trials_to_success if t <= max_trials) / max(1, n_success)
        else:
            expected = float("inf")
    ...
    def predict_growth(
        ...
        # 2. 학습률 추정
        lr_mean, lr_std = self.estimate_learning_rate(
            current_accuracy, n_responses
        )
        ...
        return GrowthPrediction(
            ...
            expected_trials=expected_trials if expected_trials != float("inf") else n_forecast_steps,
            confidence_interval=(
