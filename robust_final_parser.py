#!/usr/bin/env python3
"""
Robust final parser for mpcstudy.com MySQL dump
This parser handles the actual multi-line format found in the dump file
"""
...
class RobustFinalParser:
    def __init__(self, dump_file_path: str):
        self.dump_file_path = dump_file_path

    def parse_sql_values_robust(self, values_str: str) -> List[str]:
        """
        Parse SQL VALUES string into individual values
        Handles quoted strings, escaped quotes, NULL values, and complex content
        """
        values = []
        current_value = []
        in_quotes = False
    ...
    def extract_problems(self) -> List[ProblemData]:
        ...
        # The pattern matches: INSERT INTO `tbl_question` VALUES (values...);
        # Using DOTALL flag to match across newlines
        insert_pattern = re.compile(r"INSERT INTO `tbl_question` VALUES\s*\(([^;]+)\);", re.DOTALL)
        
        matches = insert_pattern.findall(content)
        logging.info(f"Found {len(matches)} INSERT statement matches")
        
        for i, match in enumerate(matches):
            try:
                # Parse the values string
                values = self.parse_sql_values_robust(match)
                
                if len(values) != len(TBL_QUESTION_COLUMNS):
                    logging.warning(f"Match {i+1}: Expected {len(TBL_QUESTION_COLUMNS)} columns, got {len(values)}")
                    continue
                ...
                if len(all_problems) % 100 == 0:
                    logging.info(f"Processed {len(all_problems)} problems...")
                    
            except Exception as e:
                logging.error(f"Error processing match {i+1}: {e}")
                continue
...
if __name__ == "__main__":
    ...
    parser = RobustFinalParser(dump_file)
