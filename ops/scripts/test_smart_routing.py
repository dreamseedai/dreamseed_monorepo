...
def test_language_checks():
    ...
    for lang, expect_chinese, expect_local, expect_cloud in test_cases:
        ...
        status = "✓" if (chinese == expect_chinese and 
                        local == expect_local and 
                        cloud == expect_cloud) else "✗"
        
        print(f"{status} {lang:10} → Chinese={chinese}, Local={local}, Cloud={cloud}")
        if status == "✗":
            print(f"  Expected: Chinese={expect_chinese}, Local={expect_local}, Cloud={expect_cloud}")
...
def test_routing_decision():
    ...
    for lang, expected_type, expected_model in test_cases:
        ...
        status = "✓" if (actual_type == expected_type and model == expected_model) else "✗"
