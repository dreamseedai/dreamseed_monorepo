...
def require_policy(
    ...
    def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            if request is None:
                logger.error("Request object not found in function %s arguments", func.__name__)
                raise HTTPException(status_code=500, detail="Internal Server Error: Request not found")
            ...
            result: Dict[str, Any] = await policy_client.evaluate(policy_path, input_data)
            ...
            if not allowed:
                ...
                logger.warning("Access denied by policy '%s'. Reason: %s", policy_path, reason or "No reason provided")
                raise HTTPException(
                    status_code=deny_status_code,
                    detail={
                        "error": "Policy violation",
                        "policy": policy_path,
                        "reason": result.get("reason", f"Access denied by policy '{policy_path}'")
                    }
                )

            # 정책 통과 시 로깅 및 원본 함수 실행
            logger.info("Policy '%s' check passed, proceeding to original function", policy_path)
