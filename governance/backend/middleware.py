...
class PolicyEnforcementMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, excluded_paths: Optional[List[str]] = None):
        ...
        self.excluded_paths = (
            ...
            else {"/health", "/metrics", "/docs", "/openapi.json"}
        )
        # 로거 설정
    ...
    async def dispatch(
        ...
        # 요청 시작 로깅
        self.logger.info(
            f"정책 검사 시작 - 경로: {request.url.path}, 메소드: {request.method}"
        )
        ...
        if self._should_evaluate(request):
            ...
            try:
                # 정책 엔진 평가 호출 (비동기)
                result = await get_policy_client().evaluate(
                    "dreamseedai.access_control.allow", input_data
                )
            except Exception as e:
                # 평가 호출 중 오류 발생 - 로그 기록 및 기본 거부 처리
                self.logger.error(
                    f"정책 평가 오류 - 경로: {request.url.path}, 에러: {e}"
                )
            ...
            if not allow_flag:
                # 정책 거부 로깅
                self.logger.warning(
                    f"정책 거부 - 경로: {request.url.path}, 사용자: {getattr(request.state, 'user', {})}"
                )
                ...
                # 정책 허용 로깅
                self.logger.info(
                    f"정책 허용 - 경로: {request.url.path}, 사용자: {getattr(request.state, 'user', {})}"
                )
