...
def mock_opa_deny():
    ...
    with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post:
        mock_response = AsyncMock()
        mock_response.json.return_value = {"result": {"allow": False, "reason": "Policy denied"}}
        mock_response.raise_for_status = Mock()
        mock_post.return_value = mock_response
        yield mock_post
...
class TestPolicyClientAuditIntegration:
    ...
    async def test_evaluate_logs_error(self, caplog):
        ...
        with patch("httpx.AsyncClient.post", side_effect=Exception("OPA connection failed")):
            client = PolicyEngineClient()
            input_data = {
            ...
            result = await client.evaluate("dreamseedai.data_protection.allow", input_data)
            ...
            log_data = json.loads(record.getMessage())
            assert log_data["result"] == "error"
            assert "OPA connection failed" in log_data["reason"] or "Unexpected error" in log_data["reason"]
    ...
    async def test_default_user_values(self, caplog, mock_opa_allow):
        ...
        client = PolicyEngineClient()
        input_data = {
            "resource": {"path": "/api/public", "method": "GET"}
        }
