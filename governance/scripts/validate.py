...
def additional_validations(bundle: dict) -> tuple[bool, list[str]]:
    ...
    if approvals.get("enabled"):
        ...
        for rule in rules:
            ...
            if sla_hours > 720:  # 30일
                warnings.append(
                    f"Rule '{rule['id']}' has very long SLA: {sla_hours} hours"
                )
            ...
            if rbac.get("enabled") and approver_role:
                if approver_role not in role_names:
                    errors.append(
                        f"Rule '{rule['id']}' references unknown role: {approver_role}"
                    )
    ...
    if safety.get("enabled"):
        ...
        # 시험 중 튜터 정책 검증
        if (
            tutor.get("allow_during_exam")
            and tutor.get("response_mode_during_exam") == "full"
        ):
            warnings.append(
                "Allowing full tutor access during exam may compromise academic integrity"
            )
    ...
    if privacy.get("enabled"):
        ...
        if at_rest and "AES-256" not in at_rest:
            warnings.append(
                f"Weak encryption at rest: {at_rest}. Recommend AES-256-GCM"
            )

        in_transit = encryption.get("in_transit", "")
        if in_transit and "TLS-1.3" not in in_transit:
            warnings.append(
                f"Weak encryption in transit: {in_transit}. Recommend TLS-1.3"
            )
